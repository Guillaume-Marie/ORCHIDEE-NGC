...
!!
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_sechiba/chemistry.f90 $
!! $Date: 2024-02-16 12:35:20 +0100 (ven. 16 févr. 2024) $
!! $Revision: 8428 $
!! \n
!_ ================================================================================================================================
...
MODULE chemistry
  ...
  USE ioipsl_para 
  USE xios_orchidee
  USE function_library, ONLY :lai_to_biomass 
...
!$OMP THREADPRIVATE(flx_formic)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_DMS            !! DMS emissions from vegetation (kg.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_DMS)
  REAL(r_std), ALLOCATABLE, SAVE, DIMENSION(:,:) :: flx_H2S            !! H2S emissions from vegetation (kg.m^{-2}.s^{-1})
!$OMP THREADPRIVATE(flx_H2S)
...
!_ ================================================================================================================================
    ...
    ALLOCATE (flx_DMS(kjpindex,nvm), stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'chemistry_init','Problem in allocate of variable flx_DMS','','')
    flx_DMS(:,:) = 0.

    ALLOCATE (flx_H2S(kjpindex,nvm), stat=ier)
    IF (ier /= 0) CALL ipslerr_p(3,'chemistry_init','Problem in allocate of variable flx_H2S','','')
    flx_H2S(:,:) = 0.
...
!!                as single emissions.\n
!!                DMS and H2S emissions from vegetation added (J. Lathiere, LSCE, August 2022).
!!                They follow MEGAN model for acetone, adapting EF with global estimates (highly
!!                uncertain).\n
...
!!                          :: flx_acetic, :: flx_formic, :: flx_DMS, :: flx_H2S, :: flx_no_soil, :: flx_no,
...
END IF
    ...
    DO jv = 1, nvm ! loop over the PDFs
       DO ji = 1, kjpindex ! loop over the grid cell
          ...
          ! Changes the IF-statement so it also works for
          ! a dynamic sla
!!$          IF ( sla(jv) .eq. zero ) THEN
          IF ( veget_max(ji,jv) .eq. zero ) THEN
             fol_dens(ji,jv) = zero
          ELSE
             ! 2 factor for conversion from gC to gDM. Use
             ! lai_to_biomass to account for dynamic sla
             fol_dens(ji,jv) = lai_to_biomass(2 * lai(ji,jv),jv)
          ENDIF
          ...
          IF ( ok_radcanopy ) THEN
             ...
             IF (ok_multilayer) THEN 
                ...
                !! 6.19 Calculation of DMS biogenic emissions
                flx_DMS(ji,jv) = ((1-LDF_acet)*Ct_acet(ji)*1e-9/one_hour*fol_dens(ji,jv) +LDF_acet*GAMMA_iso_m)* &
                     & em_factor_DMS(jv)*Eff_age_VOC(ji,jv)
                !! 6.20 Calculation of H2S biogenic emissions
                flx_H2S(ji,jv) = ((1-LDF_acet)*Ct_acet(ji)*1e-9/one_hour*fol_dens(ji,jv) +LDF_acet*GAMMA_iso_m)* &
                     & em_factor_H2S(jv)*Eff_age_VOC(ji,jv)
             ...
             ELSE
                ...
                IF (lai(ji,jv) .NE. zero) THEN
                   ...
                   !! 6.19 Calculation of DMS biogenic emissions
                   flx_DMS(ji,jv) = ((1-LDF_acet)*Ct_acet(ji)+LDF_acet*GAMMA_iso)*fol_dens(ji,jv)* &
                        & em_factor_DMS(jv)*Eff_age_VOC(ji,jv)*1e-9/one_hour
                   !! 6.20 Calculation of H2S biogenic emissions
                   flx_H2S(ji,jv) = ((1-LDF_acet)*Ct_acet(ji)+LDF_acet*GAMMA_iso)*fol_dens(ji,jv)* &
                        & em_factor_H2S(jv)*Eff_age_VOC(ji,jv)*1e-9/one_hour
          ...
          ELSE
             ...
             !! 6.22 Calculation of DMS biogenic emissions
             flx_DMS(ji,jv) = fol_dens(ji,jv)*em_factor_DMS(jv)*Ylt_acet(ji)*Eff_age_VOC(ji,jv)*1e-9/one_hour
             !! 6.23 Calculation of H2S biogenic emissions
             flx_H2S(ji,jv) = fol_dens(ji,jv)*em_factor_H2S(jv)*Ylt_acet(ji)*Eff_age_VOC(ji,jv)*1e-9/one_hour
    ...
    CALL xios_orchidee_send_field("flx_DMS",flx_DMS)
    CALL xios_orchidee_send_field("flx_H2S",flx_H2S)
    ...
    IF ( .NOT. almaoutput ) THEN
       ...
       CALL histwrite_p(hist_id, 'flx_DMS', kjit, flx_DMS, kjpindex*nvm, indexveg)
       CALL histwrite_p(hist_id, 'flx_H2S', kjit, flx_H2S, kjpindex*nvm, indexveg)
       ...
       IF ( hist2_id > 0 ) THEN
          ...
          CALL histwrite_p(hist2_id, 'flx_DMS', kjit, flx_DMS, kjpindex*nvm, indexveg)
          CALL histwrite_p(hist2_id, 'flx_H2S', kjit, flx_H2S, kjpindex*nvm, indexveg)
...
!$OMP THREADPRIVATE(nb_fields_out, nb_fields_in)
     ...
     IF (ok_bvoc) THEN 
        ...
        DO i_fields_out=1,nb_fields_out
           ...
           CASE("flx_DMS")
              fields_out(:,:,i_fields_out)=flx_DMS(:,:)
           CASE("flx_H2S")
              fields_out(:,:,i_fields_out)=flx_H2S(:,:)
           CASE("flx_no_soil_YL") 
              fields_out(:,:,i_fields_out)=flx_no_soil(:,:)
           CASE("flx_nox_YL") 
              fields_out(:,:,i_fields_out)=flx_no(:,:)
           CASE("flx_fertil_no_YL") 
...
!_ ================================================================================================================================
   SUBROUTINE chemistry_clear
     ...
     IF (ALLOCATED (flx_DMS)) DEALLOCATE (flx_DMS)
     IF (ALLOCATED (flx_H2S)) DEALLOCATE (flx_H2S)
