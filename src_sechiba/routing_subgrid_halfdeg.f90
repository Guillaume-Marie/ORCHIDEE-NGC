! =================================================================================================================================
! MODULE       : routing_subgrid_halfdeg
!
...
!!
!!\n DESCRIPTION: This method computes the routing on the ORCHIDEE grid using sub-basins from a 0.5degree grid.
!!                
!!
!! RECENT CHANGE(S): July 2024: Changed routing method's name "standard" (not mentioned in the code) to "subgrid_halfdeg". 
!!
!! REFERENCE(S)	:
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_sechiba/routing_subgrid_halfdeg.f90 $
...
MODULE routing_subgrid_halfdeg
  ...
  USE sechiba_io_p
  USE interpol_help
  USE grid, ONLY : area, NbNeighb, grid_type, regular_lonlat
  USE mod_orchidee_para, ONLY : area_g


  IMPLICIT NONE
  PRIVATE
  PUBLIC :: routing_subgrid_halfdeg_main, routing_subgrid_halfdeg_initialize, routing_subgrid_halfdeg_finalize, routing_subgrid_halfdeg_clear, routing_names, routing_subgrid_halfdeg_xios_initialize
...
CONTAINS
  !!  =============================================================================================================================
  !! SUBROUTINE:         routing_subgrid_halfdeg_initialize
  !!
  !>\BRIEF	         Initialize the routing_subgrid_halfdeg module
  !!
  !! DESCRIPTION:        Initialize the routing_subgrid_halfdeg module. Read from restart file or read the routing.nc file to initialize the
  !!                     routing scheme. 
  !!
  !! RECENT CHANGE(S)
  !!
  ...
  SUBROUTINE routing_subgrid_halfdeg_initialize( kjit,        nbpt,           index,                 &
...
!_ ================================================================================================================================
    ...
    IF (is_root_prc) THEN
       ...
       IF (nb_coast_gridcells == 0)THEN
          CALL ipslerr(3,'routing_subgrid_halfdeg_initialize',&
    ...
    IF (ier /= 0) CALL ipslerr_p(3,'routing_subgrid_halfdeg_inititalize','Pb in allocate for mask_coast','','')
  ...
  END SUBROUTINE routing_subgrid_halfdeg_initialize

  !!  =============================================================================================================================
  !! SUBROUTINE:    routing_subgrid_halfdeg_xios_initialize
  !!
  ...
  END SUBROUTINE routing_subgrid_halfdeg_xios_initialize

!! ================================================================================================================================
!! SUBROUTINE   : routing_subgrid_halfdeg_main 
!!
...
SUBROUTINE routing_subgrid_halfdeg_main(kjit, nbpt, index, &
...
!_ ================================================================================================================================
    ...
    !
    !
  END SUBROUTINE routing_subgrid_halfdeg_main
  
  !!  =============================================================================================================================
  !! SUBROUTINE:         routing_subgrid_halfdeg_finalize
  !!
  ...
  SUBROUTINE routing_subgrid_halfdeg_finalize( kjit, nbpt, rest_id, flood_frac, flood_res )
...
!_ ================================================================================================================================
  ...
  END SUBROUTINE routing_subgrid_halfdeg_finalize
...
!    flood_res = zero
  ...
  !
!! ================================================================================================================================
!! SUBROUTINE 	: routing_subgrid_halfdeg_clear
!!
...
!_ ================================================================================================================================
  ...
  END SUBROUTINE routing_subgrid_halfdeg_clear
  !
...
!>
    ...
    DO ig=1,nbpt
       ...
       !
       ! The three output types for the routing : endorheic basins, rivers and
...
!!                For the moment it only works for endorheic lakes but it can be extended in the future.
!!
...
!_ ================================================================================================================================
    ...
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for pts','','')

    ALLOCATE(ptbas(num_largest, nbpt), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for ptbas','','')

    ALLOCATE(outpt(num_largest, 2), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for outpt','','')

    ALLOCATE(nb_pts(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for nb_pts','','')

    ALLOCATE(totarea(num_largest), tmparea(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for totarea','','')

    ALLOCATE(topids(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for topids','','')

    ALLOCATE(sortedrivs(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for sortedrivs','','')

    ALLOCATE(sorted_names(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for sorted_names','','')

    ALLOCATE(streams_nb(num_largest), streams_avehops(num_largest), streams_minhops(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for streams_nb','','')

    ALLOCATE(streams_maxhops(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for streams_maxhops','','')

    ALLOCATE(streams_resid(num_largest), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for streams_resid','','')

    ALLOCATE(lbasin_area(num_largest,nbpt), lbasin_uparea(num_largest,nbpt), lrivercode(num_largest,nbpt), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for lbasin_area','','')
    ...
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for basins_names','','')
    ...
    IF ( err_nbpt_grid_basin ) THEN
       ...
       CALL ipslerr(3, 'routing_diagnostic', 'We are heading for a out of bounds in arrays pts, ptsbas and lbasin_area.',&
    ...
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for allstreams','','')

    ALLOCATE(tstreams(longest_river), tupstreamchange(longest_river), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for tstreams','','')

    ALLOCATE(tslen(nbmax), tpts(nbmax), tptbas(nbmax), tuparea(nbmax), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for tslen','','')

    ALLOCATE(tcode(nbmax), stat=ier)
    IF (ier /= 0) CALL ipslerr(3,'routing_diagnostic','Pb in allocate for tcode','','')
    ...
    IF ( err_basin_number ) THEN
       CALL ipslerr(3, 'routing_diagnostic', 'We found a basin number larger than 999999.',&
          ...
          DO ic=num_largest,1,-1
             IF ( TRIM(sorted_names(icc)) == TRIM(sorted_names(ic)) .AND. name_found > 1 ) THEN
                ...
                ELSE
                   ...
                   CALL ipslerr(3, 'routing_diagnostic', &
...
!_ ================================================================================================================================
    ...
    IF (iret /= NF90_NOERR) THEN
       CALL ipslerr (3,'routing_diagncfile', 'Could not create file :', &
    ...
    IF ( grid_type == regular_lonlat ) THEN
       ...
       IF (iret /= NF90_NOERR) THEN
          CALL ipslerr (3,'routing_diagncfile', 'Dimension "x" can not be defined for the file : ', &
               &         TRIM(river_file_name),'(Solution ?)')
       ENDIF
       iret = NF90_DEF_DIM(fid, 'lat', jjm_g, dims(2))
       IF (iret /= NF90_NOERR) THEN
          CALL ipslerr (3,'routing_diagncfile', 'Dimension "y" can not be defined for the file : ', &
               &         TRIM(river_file_name),'(Solution ?)')
       ENDIF
    ELSE
       !
    ...
    IF ( grid_type == regular_lonlat ) THEN
       ...
       IF (iret /= NF90_NOERR) THEN
          CALL ipslerr (3,'routing_diagncfile', 'Variable '//lon_name//' can not be defined for the file : ', &
               &         TRIM(river_file_name),'(Solution ?)')
       ENDIF
       !
       lat_name = 'lat'
       ...
       IF (iret /= NF90_NOERR) THEN
          CALL ipslerr (3,'routing_diagncfile', 'Variable '//lat_name//' can not be defined for the file : ', &
               &         TRIM(river_file_name),'(Solution ?)')
       ENDIF
       !
    ELSE
    ...
    IF (iret /= NF90_NOERR) THEN
       CALL ipslerr (3,'routing_diagncfile', 'Could not add attribut to variable '//lon_name//' for the file :', &
            &          TRIM(river_file_name),'(Solution ?)')
    ENDIF
    !
    lon_min = -180.
    ...
    IF (iret /= NF90_NOERR) THEN
       CALL ipslerr (3,'routing_diagncfile', 'Could not add attribut to variable '//lat_name//' for the file :', &
            &          TRIM(river_file_name),'(Solution ?)')
    ENDIF
    !
    lat_max = 90.
    ...
    IF (iret /= NF90_NOERR) THEN
       CALL ipslerr (3,'routing_diagncfile', 'Could not end definitions in the file : ', &
 &          TRIM(river_file_name),'(Solution ?)')
    ENDIF
    !
    ...
    IF ( grid_type == regular_lonlat ) THEN
       ...
       IF (iret /= NF90_NOERR) THEN
          CALL ipslerr (3,'routing_diagncfile', 'Could not put variable nav_lon  in the file : ', &
               &          TRIM(river_file_name),'(Solution ?)')
       ENDIF
       !
       iret = NF90_PUT_VAR(fid, nlatid, lat_g(1,1:jjm_g))
       IF (iret /= NF90_NOERR) THEN
          CALL ipslerr (3,'routing_diagncfile', 'Could not put variable nav_lat  in the file : ', &
               &          TRIM(river_file_name),'(Solution ?)')
       ENDIF
    ELSE
       !
    ...
    DO icc = 1, num_largest
       ...
       IF ( nb_pts(ib) > 2 ) THEN
          ...
          DO ij=1, nb_pts(ib)
             ...
             basinfrac(i,j) = basinfrac(i,j) + lbasin_area(ib,ij)/area_g(ik)
          ...
          iret = NF90_REDEF(fid)
          IF (iret /= NF90_NOERR) THEN
             CALL ipslerr (3,'routing_diagncfile', &
          ...
          IF (iret /= NF90_NOERR) THEN
             CALL ipslerr (3,'routing_diagncfile', 'Variable '//TRIM(var_name)//' can not be defined for the file : ', &
                  &         TRIM(river_file_name),'(Solution ?)')
          ENDIF
          !
          ierr_tot = 0
          ...
          !
          IF (ierr_tot > 0 ) THEN
             CALL ipslerr (3,'routing_diagncfile', 'Could not add some attributes to variable '//var_name//' for the file :', &
                  &          TRIM(river_file_name),'(Solution ?)')
          ENDIF
          !
          ...
          IF (iret /= NF90_NOERR) THEN
             CALL ipslerr (3,'routing_diagncfile', 'Could not put variable basinfrac in the file : ', &
                  &          TRIM(river_file_name),'(Solution ?)')
          ENDIF
          ...
          IF (iret /= NF90_NOERR) THEN
             CALL ipslerr (3,'routing_diagncfile', 'Could not put variable basinuparea in the file : ', &
                  &          TRIM(river_file_name),'(Solution ?)')
          ENDIF
          ...
          IF (iret /= NF90_NOERR) THEN
             CALL ipslerr (3,'routing_diagfile', 'Could not put variable basincode in the file : ', &
                  &          TRIM(river_file_name),'(Solution ?)')
          ENDIF
          !
       ENDIF
...
!_ ================================================================================================================================
    ...
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for lat_rel','','')

    ALLOCATE(lon_rel(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for lon_rel','','')

    ALLOCATE (trip(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for trip','','')

    ALLOCATE (basins(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basins','','')

    ALLOCATE (topoindex(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for topoindex','','')

    ALLOCATE (hierarchy(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for hierarchy','','')
    ...
    DO ip=1,iml
       DO jp=1,jml
          IF ( trip(ip,jp) < 1.e10 .AND. topoindex(ip,jp) > 1.e10) THEN
             ...
             CALL ipslerr(3,'routing_basins','trip exists but not topoind','','')
          ENDIF
       ENDDO
    ENDDO

    ALLOCATE(resol_lu(iml,jml,2), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for resol_lu','','')

    ALLOCATE(mask(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for mask','','')
    !
    ...
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for sub_area','','')
    sub_area(:,:)=zero

    ALLOCATE (sub_index(nbpt,nbvmax,2), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for sub_index','','')
    sub_index(:,:,:)=0

    ALLOCATE (sub_pts(nbpt), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for sub_pts','','')
    sub_pts(:)=0
    !
    ...
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for area_bx','','')
    ALLOCATE (hierarchy_bx(nbvmax,nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for hierarchy_bx','','')
    ALLOCATE (lon_bx(nbvmax,nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for lon_bx','','')
    ALLOCATE (lat_bx(nbvmax,nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for lat_bx','','')
    ALLOCATE (topoind_bx(nbvmax,nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for topoind_bx','','')
    ALLOCATE (trip_bx(nbvmax,nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for trip_bx','','')
    ALLOCATE (basin_bx(nbvmax,nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_bx','','')
    ALLOCATE (coast_pts(nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for coast_pts','','')
    ALLOCATE (basin_inbxid(nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_inbxid','','')
    ALLOCATE (basin_sz(nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_sz','','')
    ALLOCATE (basin_pts(nbvmax,nbvmax,2), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_pts','','')
    ALLOCATE (basin_bxout(nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_bxout','','')
    ALLOCATE (basin_count(nbpt), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_count','','')
    ALLOCATE (basin_area(nbpt,nwbas), basin_hierarchy(nbpt,nwbas), basin_topoind(nbpt,nwbas), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_area','','')
    ALLOCATE (fetch_basin(nbpt,nwbas), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for fetch_basin','','')
    ALLOCATE (basin_id(nbpt,nwbas),  basin_flowdir(nbpt,nwbas), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for basin_id','','')
    ALLOCATE (outflow_grid(nbpt,nwbas),outflow_basin(nbpt,nwbas), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for outflow_grid','','')
    ALLOCATE (inflow_number(nbpt,nwbas), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for inflow_number','','')
    ALLOCATE (inflow_basin(nbpt,nwbas,nbvmax), inflow_grid(nbpt,nwbas,nbvmax), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for inflow_basin','','')
    ALLOCATE (nbcoastal(nbpt), coastal_basin(nbpt,nwbas), stat=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr(3,'routing_basins','Pb in allocate for nbcoastal','','')
...
!_ ================================================================================================================================
    ...
    ELSE
       ...
       area_bx(iloc, jloc) = area_g(ib)*contfrac(ib)
...
!_ ================================================================================================================================
    ...
    ELSE
       ...
       CALL ipslerr(3,'routing_sortcoord','The chosen direction is not recognized','First section','')
    ENDIF
    !
    ...
    ELSE
       ...
       CALL ipslerr(3,'routing_sortcoord','The chosen direction is not recognized','Second section','')
    ENDIF
    !
...
!_ ================================================================================================================================
       ...
       DO jp=1,nbj
          IF ( basin(ip,jp) .LT. undef_int) THEN
             IF ( COUNT(basin(ip,jp) .EQ. bname(:)) .EQ. 0 ) THEN
                nbb = nbb + 1
                IF ( nbb .GT. nbvmax ) CALL ipslerr(3,'routing_findbasins','nbvmax too small','first section','')
             ...
             !
             sz(ibas) = sz(ibas) + 1
             IF ( sz(ibas) .GT. nbvmax ) CALL ipslerr(3,'routing_findbasins','nbvmax too small','second section','')
    ...
    ELSE
       ...
       CALL ipslerr(3,'routing_findbasins','Increase nbvmax.','It is too small to contain all the basins','')
    ENDIF
    !
    ...
    DO ip=1,nbb
       IF ( sz(ip) .GT. 0 ) THEN
          ...
          ELSE
             ...
             ENDDO
             CALL ipslerr(3,'routing_findbasins','Probleme finding trip','','')
          ENDIF
       ENDIF
    ENDDO
    !
...
!!    IF ( COUNT(trip(1:nbi,1:nbj) .EQ. 98) .GT. 0) ip = ip + 1
    IF ( ip .LT. nb_basin ) THEN
       ...
       CALL ipslerr(3,'routing_findbasins','Probleme less outflow points than basins','','')
    ENDIF
    ...
    IF ( COUNT(outsz(1:nbout) .GT. 0) .GT. 1 ) THEN
       ...
       IF ( SUM(sz(nb_in+1:nb)) .NE. basin_sz) THEN
          ...
          DO jp=1,nbj
             WRITE(numout,fmt) trip_tmp(1:nbi,jp)
          ENDDO
          CALL ipslerr(3,'routing_cutbasin','Lost some points while spliting the basin','','')
       ENDIF
       ...
       IF ( debug ) THEN
          ...
          IF ( MAXVAl(trip_tmp(1:nbi,1:nbj)) .GT. 0) THEN
             CALL ipslerr(3,'routing_cutbasin','Error in debug checking','','')
          ENDIF
       ENDIF
    ENDIF
    !
       ...
       DO jp=1,jml
          IF ( trip(ip,jp) .LT. undef_sechiba ) THEN
             ...
             IF (  topoindex(ip,jp) .GT. 1.e10 ) THEN
                ...
                CALL ipslerr(3,'routing_hierarchy','Too large topographic index','','')
             ELSE
             ...
             DO WHILE ( trp .GT. 0 .AND. trp .LT. 9 .AND. cnt .LT. iml*jml)
                ...
                IF ( topohier_old .GT. topohier) THEN
                   ...
                   CALL ipslerr(3,'routing_hierarchy','Big Problem, how comes we climb up a hill ?','','')
                ENDIF
             ...
             IF ( cnt .EQ. iml*jml) THEN
                ...
                ENDDO
                CALL ipslerr(3,'routing_hierarchy','We could not route point','','')
             ENDIF
...
!_ ================================================================================================================================
       ...
       DO jp=1,nbj
          IF ( (trip_flow(ip,jp,1) + trip_flow(ip,jp,2)) .GT. 0) THEN
             ...
             IF ( cnt .EQ. nbi*nbj) THEN
                ...
                ENDDO
                CALL ipslerr(3,'routing_findrout','We could not route point','','')
             ENDIF
          ENDIF
       ENDDO
    ENDDO
    ...
    IF ( basin_sz .NE. totsz) THEN
       ...
       DO jp=1,nbj
          WRITE(numout,fmt) trip(1:nbi,jp)
       ENDDO
       CALL ipslerr(3,'routing_findrout','Water got lost while I tried to follow it','','')
    ENDIF
    !
       ...
       DO iz=1,basin_sz(ij)
          ...
          SELECT CASE (hierar_method)
             ...
             CASE DEFAULT
                ...
                CALL ipslerr(3,'routing_globalize','Unknown method for computing the hierarchy of the basin','','')
          END SELECT
          !
       ENDDO
       !
       ...
       DO sb=1,basin_count(sp)
          ...
          IF ( inp .GE. 0 ) THEN
             IF ( basin_count(inp) .LT. sbl ) THEN
                ...
                CALL ipslerr(3,'routing_linkup','Problem with outflow','','')
             ENDIF
          ENDIF
       ENDDO
    ENDDO
...
!_ ================================================================================================================================
    ...
    DO ib=1,nbpt
       ...
       contarea = area_g(ib)*contfrac(ib)
       ...
       DO ij=1,basin_count(ib)
          ...
          DO WHILE (igrif .GT. 0)
             ...
             IF ( itt .GT. 500) THEN
                ...
                IF ( itt .GT. 510) THEN
                   CALL ipslerr(3,'routing_fetch','Problem...','','')
                ENDIF
             ENDIF
          ENDDO
          !
    ...
    IF ( COUNT(basin_count .GT. nbasmax) .GT. 0 ) THEN
       ...
       IF ( COUNT(basin_count .GT. nbasmax)/nbpt*100 .GT. 5 ) THEN
          ...
          DO ib = 1, nbpt
             ...
             ENDIF
          ENDDO
          CALL ipslerr(3,'routing_truncate','No basin found which could be supressed.','','')
       ELSE
          !
          !
          DO ib = 1,nbpt
    ...
    DO ib=1,nbpt
       DO ij=1,basin_count(ib)
          ...
          IF ( ijf .GT. basin_count(ibf) .AND.  ijf .LE. nbasmax) THEN
             ...
             CALL ipslerr(3,'routing_truncate','Problem with routing..','','')
          ENDIF
       ENDDO
    ENDDO
    !
...
!       IF ( route_tobasin(ff(1), ff(2)) .EQ. nbasmax + 2) THEN
    ...
    IF ( ibf .LT.  num_largest) THEN
       ...
       CALL ipslerr(3,'routing_truncate','Not enough basins','','')
    ENDIF
    !
    !
    !
...
!_ ================================================================================================================================
    ...
    IF ( outflow_grid(ib,tokill) .GT. 0) THEN
       ...
       IF ( .NOT. doshift ) THEN
          ...
          CALL ipslerr(3,'routing_killbas','Basin not found','','')
       ENDIF
...
!!$          ENDIF
       ...
       !
       IF ( init_swamp ) THEN
          swamp(ib) = MIN(area_flood(iswamp), area(ib)*contfrac(ib))
          IF ( swamp(ib) < 0 ) THEN
...
!$OMP THREADPRIVATE(totw_coastal)
    REAL(r_std)                :: totarea              !! Total area of basin (m^2)
    REAL(r_std)                :: area_tot             !! Total area of routing (m^2)
    INTEGER(i_std)             :: ig                   !!
    !
...
!_ ================================================================================================================================
    ...
    ELSE
       ...
       IF ( ABS((totw_flood + totw_stream + totw_fast + totw_slow + totw_lake + totw_pond) - &
          ...
          WRITE(numout,*) 'WARNING : Water not conserved in routing_subgrid_halfdeg. Limit at ', allowed_err, ' 10^6 kg'
          ...
          WRITE(numout,*) '--Water>>> change by fluxes : ', totw_out - totw_in, ' Diff [mm/dt]: ',   &
               & ((totw_flood + totw_stream + totw_fast + totw_slow + totw_lake) - (totw_out - totw_in))/area_tot

          ! Stop the model
          CALL ipslerr_p(3, 'routing_waterbal', 'Water is not conserved in routing_subgrid_halfdeg.','','')
       ENDIF
       !
    ENDIF
    !
  ...
  !
  !
END MODULE routing_subgrid_halfdeg
