...
!! continental surfaces scheme. The module contains the following subroutines: enerbil_initialize, enerbil_main,
...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_sechiba/enerbil.f90 $
...
!_ ================================================================================================================================
    ...
    printlev_loc=get_printlev('enerbil')
  ...
  SUBROUTINE enerbil_main (kjit, kjpindex, netrad, lwabs, lwnet, fluxsubli, index, &
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: resist_equiv_evap_veg   !! Equivalent resistances to the fluxes involved in the evaporation 
                                                                                    !! flux from the vegetation (bare soil evaporation + interception loss 
                                                                                    !! + transpiration) (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: resist_equiv_evap       !! Equivalent resistances to the fluxes involved in the evaporation 
                                                                                    !! flux (bare soil evaporation + interception loss + transpiration + 
                                                                                    !! floodplains evaporation) (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: resist_equiv_tot        !! Equivalent resistances to the fluxes involved in the total 
                                                                                    !! evaporative flux (all evaporations + snow sublimation) (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: frac_sub_snow           !! Fraction of gridcell where snow sublimation occurs (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: frac_evap_bare          !! Fraction of gridcell where bare soil evaporation occurs (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)        :: frac_evap_flood         !! Fraction of gridcell where floodplains evaporation occurs (-)
    ...
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)    :: frac_evap_intercept   !! Fraction of gridcell where intercepted water evaporation occurs (-)
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)    :: resist_intercept      !! Sum of the resistances applied to intercepted water evaporation (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)    :: frac_evap_transp      !! Fraction of gridcell where transpiration occurs (-)
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)    :: resist_transp         !! Sum of the resistances applied to transpiration (s m^{-1})
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (out)       :: qair_new         !! New specific humidity at lowest level (kg kg^{-1})
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (out)       :: lwabs            !! LW radiation absorbed by the surface (W m^{-2})
    REAL(r_std),DIMENSION (kjpindex), INTENT (out)       :: lwnet            !! Net Long-wave radiation (W m^{-2})
    REAL(r_std),DIMENSION (kjpindex), INTENT (out)       :: fluxsubli        !! Energy of sublimation (mm day^{-1})
...
!_ ================================================================================================================================
    ...
    CALL enerbil_begin (kjpindex, temp_sol, lwdown, swnet, pb, psold, qsol_sat, pdqsold, netrad, &
       & emis, lwabs)
    ...
    CALL enerbil_surftemp (kjpindex, emis, &
       & epot_air, petAcoef, petBcoef, qair, peqAcoef, peqBcoef, soilflx, rau, u, v, q_cdrag, resist_equiv_evap, resist_equiv_evap_veg,&
       & frac_sub_snow, frac_evap_flood, raero, soilcap, lwdown, swnet, netrad, psnew, qsol_sat_new, temp_sol_new, &
    ...
    CALL enerbil_flux (kjpindex, emis, temp_sol, rau, u, v, q_cdrag, resist_equiv_evap, frac_sub_snow, frac_evap_flood, &
       ...
       & precip_rain, snowdz, temp_air, pgflux, soilcap, temp_sol_add, qsol_sat_new, raero, resist_equiv_tot)
    ...
    CALL enerbil_evapveg (kjpindex, frac_sub_snow, frac_evap_intercept, resist_intercept, frac_evap_transp, resist_transp, &
       & vbeta3pot, frac_evap_bare, frac_evap_flood, raero,  &
...
!_ ================================================================================================================================
  ...
  SUBROUTINE enerbil_begin (kjpindex, temp_sol, lwdown, swnet, pb, psold, qsol_sat, pdqsold, &
                            netrad, emis, lwabs)
...
!_ ================================================================================================================================
 ...
 &           'pdqsold too high ','','')
  ...
  SUBROUTINE enerbil_surftemp (kjpindex, emis, epot_air, &
     & petAcoef, petBcoef, qair, peqAcoef, peqBcoef, soilflx, rau, u, v, q_cdrag, resist_equiv_evap, resist_equiv_evap_veg, &
     & frac_sub_snow, frac_evap_flood, raero, soilcap, lwdown, swnet, netrad, psnew, qsol_sat_new, temp_sol_new, &
     & qair_new, epot_air_new)
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: q_cdrag       !! Surface drag coefficient  (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: resist_equiv_evap     !! Equivalent resistances to the fluxes involved in the evaporation 
                                                                                      !! flux (bare soil evaporation + interception loss + transpiration) (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: resist_equiv_evap_veg !! Equivalent resistances to the fluxes involved in the evaporation 
                                                                                      !! flux from the vegetation (bare soil evaporation + interception loss   
                                                                                      !! + transpiration) (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: frac_sub_snow         !! Fraction of the gridcell where snow sublimation occurs (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: frac_evap_flood       !! Fraction of the gridcell where flodplains evaporation occurs (-)
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: lwdown        !! Down-welling long-wave flux (W m^{-2})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: swnet         !! Net surface short-wave flux (W m^{-2})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: netrad        !! Net radiation (W m^{-2})
...
!_ ================================================================================================================================
    ...
    !! 2. Computation of fluxes for the old surface conditions
      ...
      larsub_old = chalsu0 * rau(ji) * frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * (un / raero(ji)) * &
           (peqBcoef(ji) -  qsol_sat(ji)) / (un - frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * rau(ji) * peqAcoef(ji) / raero(ji))

      !! \latexonly 
      !!     \input{enerbilsurftemp5.tex}
      !! \endlatexonly

      !! If the resistance is near infinity, we should not take the flux into account
      IF ( (un / resist_equiv_evap_veg(ji)) .GE. 10.0**(-20) ) THEN
         lareva_old = chalev0 * rau(ji) * (un / resist_equiv_evap_veg(ji)) * (peqBcoef(ji) -  qsol_sat(ji)) / &
              (un - (un / resist_equiv_evap_veg(ji)) * rau(ji) * peqAcoef(ji)) + &
              (chalev0 * rau(ji) * frac_evap_flood(ji) * (peqBcoef(ji) -  qsol_sat(ji)) / raero(ji)) / &
              (un - frac_evap_flood(ji) * rau(ji) * peqAcoef(ji) / raero(ji))
      ELSE
         lareva_old = (chalev0 * rau(ji) * frac_evap_flood(ji) * (peqBcoef(ji) -  qsol_sat(ji)) / raero(ji)) / &
              (un - frac_evap_flood(ji) * rau(ji) * peqAcoef(ji) / raero(ji))
      ENDIF
    ...
    !! 3. Calculation of sensitivity terms
      ...
      larsub_sns = chalsu0 * rau(ji) * frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * zicp(ji) * &
           pdqsold(ji) * (un / raero(ji)) / (un - frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * rau(ji) * peqAcoef(ji) / raero(ji))

      !! lareva_sns is the latent heat sensitivity for evaporation. vbeta1 (src_sechiba/diffuco_snow) 
      !! and vbeta (src_sechiba/diffuco_comb) are the evaporation coefficients.
      !! \latexonly 
      !!     \input{enerbilsurftemp9.tex}
      !! \endlatexonly

      !! If the resistance is near infinity, we should not take the flux into account.
      IF ( (un / resist_equiv_evap(ji)) .GE. 10.0**(-20) ) THEN
         lareva_sns = chalev0 * rau(ji) * (un / resist_equiv_evap(ji)) * zicp(ji) * pdqsold(ji) / &
              (un -  rau(ji) * (un / resist_equiv_evap(ji)) * peqAcoef(ji))
      ELSE
         lareva_sns = zero
      ENDIF
    ...
    !! 4. Solution of the energy balance
      ...
      ELSE
        ...
        !! \endlatexonly    
         ...
         !! If the resistance is near infinity, we should not take the flux into account
         IF ( (un / resist_equiv_evap(ji)) .GE. 10.0**(-20) ) THEN
            qair_new(ji) = un / ( chalsu0 * rau(ji) * frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * (un / raero(ji)) + &
                 & chalev0 * rau(ji) * (un / resist_equiv_evap(ji)) ) &
 ...
 !_ ==============================================================================================================================  
  ...
  SUBROUTINE enerbil_pottemp (kjpindex, zlev, emis, epot_air, &
     & petAcoef, petBcoef, qair, peqAcoef, peqBcoef, soilflx, rau, u, v, q_cdrag, resist_equiv_evap,&
     & frac_sub_snow, soilcap, lwdown, swnet, q_sol_pot, temp_sol_pot) 
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE enerbil_flux (kjpindex, emis, temp_sol, rau, u, v, q_cdrag, resist_equiv_evap, frac_sub_snow, frac_evap_flood, &
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: resist_equiv_evap  !! Equivalent resistances to the fluxes involved in the evaporation 
                                                                                   !! flux (bare soil evaporation + interception loss + transpiration) (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: frac_sub_snow      !! Fraction of gridcell on which snow sublimation occurs (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: frac_evap_flood    !! Fraction of gridcell on which floodplains evaporation occurs (-) 
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: qair          !! Lowest level specific humidity (kg kg^{-1})
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: epot_air      !! Air potential energy (J)
    ...
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: qsol_sat_new  !! New saturated surface air moisture (kg kg^{-1})     
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: raero         !! Aerodynamic resistance (s m^{-1})  
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)            :: resist_equiv_tot  !! Equivalent resistances to the fluxes involved in the total evaporative 
                                                                                  !! flux (all evaporations + snow sublimation) (s m^{-1})
...
!_ ================================================================================================================================
    ...
    !! 2. Calculation of the net upward flux of longwave radiation
      ...
      IF ( (un / resist_equiv_tot(ji)) .GE. 10.0**(-20) ) THEN
         qsurf(ji) = raero(ji) / resist_equiv_tot(ji) * qsol_sat_new(ji) + (un - raero(ji) / resist_equiv_tot(ji)) * qair(ji)
      ELSE
         qsurf(ji) = zero
      ENDIF
      ...
      !! If the resistance is near infinity, we should not take the flux into account.
      IF ( (un / resist_equiv_tot(ji)) .GE. 10.0**(-20) ) THEN
         vevapp(ji) = dt_sechiba * rau(ji) * (un / resist_equiv_tot(ji)) * (qsol_sat_new(ji) - qair(ji))
      ELSE
         vevapp(ji) = zero
      ENDIF
      ...
      !! If the resistance is near infinity, we should not take the flux into account
      IF ( (un / resist_equiv_evap(ji) ) .GE. 10.0**(-20)) THEN
         fluxlat(ji) = chalsu0 * rau(ji) * frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * &
              & (qsol_sat_new(ji) - qair(ji)) / raero(ji) + &
              &  chalev0 * rau(ji) * (qsol_sat_new(ji) - qair(ji)) / resist_equiv_evap(ji)
      ELSE
         fluxlat(ji) = chalsu0 * rau(ji) * frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * &
           & (qsol_sat_new(ji) - qair(ji)) / raero(ji)
      ENDIF
      ...
      fluxsubli(ji) = chalsu0 * rau(ji) * frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * &
           & (qsol_sat_new(ji) - qair(ji)) / raero(ji)
      ...
      !! \endlatexonly
      fluxsens(ji) =  rau(ji) * (psnew(ji) - epot_air(ji)) / raero(ji)
      ...
      evapot(ji) =  MAX(zero, dt_sechiba * rau(ji) * (qsol_sat_new(ji) - qair(ji)) / raero(ji))
      ...
      IF (temp_sol_new (ji) > tp_00 .AND. &
          soilcap(ji) .GT. min_sechiba) THEN
         ...
         !! If the resistance is near infinity, we should not take the flux into account
         IF ( (un / resist_equiv_evap(ji) ) .GE. 10.0**(-20) ) THEN
            fluxlat_tmp(ji) = chalsu0 * rau(ji) * frac_sub_snow(ji) * (un-frac_evap_flood(ji)) * &
                 & (qsol_sat_tmp(ji) - qair(ji)) / raero(ji) + &
                 &  chalev0 * rau(ji) * (qsol_sat_tmp(ji) - qair(ji)) / resist_equiv_evap(ji)
         ELSE
            fluxlat(ji) = chalsu0 * rau(ji) * frac_sub_snow(ji) * (un - frac_evap_flood(ji)) * &
                 & (qsol_sat_new(ji) - qair(ji)) / raero(ji)
         ENDIF 
  ...
  SUBROUTINE enerbil_evapveg (kjpindex, frac_sub_snow, frac_evap_intercept, resist_intercept, &
     & frac_evap_transp, resist_transp, vbeta3pot, frac_evap_bare, frac_evap_flood, raero, &
    ...
    INTEGER(i_std), INTENT(in)                          :: kjpindex          !! Domain size (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)       :: frac_sub_snow     !! Fraction of gridcell on which snow sublimation occurs (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)       :: frac_evap_bare    !! Fraction of gridcell on which bare soil evaporation occurs (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)       :: frac_evap_flood   !! Fraction of gridcell on which floodplains evaporation occurs (-)
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)       :: rau               !! Density (kg m^{-3})
...
!!$ DS 15022011 humrel was used in a previous version of Orchidee, developped by Nathalie. Need to be discussed if it should be introduce again             
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)   :: frac_evap_intercept  !! Fraction of gridcell where intercepted water evaporation occurs (-)          
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)   :: resist_intercept     !! Sum of the resistances applied to intercted water evaporation flux (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)   :: frac_evap_transp     !! Fraction of gridcell where transpiration occurs (-)            
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)   :: resist_transp        !! Sum of the resistances applied to transpiration (s m^{-1})         
    REAL(r_std),DIMENSION (kjpindex), INTENT (in)       :: raero             !! Aerodynamic resistance (s m^{-1})
    REAL(r_std),DIMENSION (kjpindex,nvm), INTENT (in)   :: vbeta3pot         !! Vegetation resistance for potential transpiration
...
!_ ==============================================================================================================================
    ...
    DO ji=1,kjpindex 
      ...
      vevapsno(ji) = (un - frac_evap_flood(ji)) * frac_sub_snow(ji) * dt_sechiba * &
           rau(ji) *  (qsol_sat_new(ji) - qair(ji)) / raero(ji)
      ...
      vevapnu(ji) =  (un - frac_evap_flood(ji)) * (un - frac_sub_snow(ji)) * frac_evap_bare(ji) * &
           rau(ji) * dt_sechiba * (qsol_sat_new(ji)-qair(ji)) / raero(ji)

      !
      ! 1.3 floodplains evaporation - transpiration et interception prioritaires dans les floodplains
      !

      vevapflo(ji) = frac_evap_flood(ji) * dt_sechiba * rau(ji) *  (qsol_sat_new(ji) - qair(ji)) / raero(ji)


    !! 2. Compute transpir (transpiration) and vevapwet (interception) 

      DO jv=1,nvm
         vevapwet(ji,jv) = (un - frac_evap_flood(ji)) * (un - frac_sub_snow(ji)) * frac_evap_intercept(ji,jv) * &
              rau(ji) * dt_sechiba * (qsol_sat_new(ji)-qair(ji)) / resist_intercept(ji,jv)

         transpir(ji,jv) =  (un - frac_evap_flood(ji)) * (un - frac_sub_snow(ji)) * frac_evap_transp(ji,jv) * &
              rau(ji) * dt_sechiba * (qsol_sat_new(ji)-qair(ji)) / resist_transp(ji,jv)
        
         transpot(ji,jv) = vbeta3pot(ji,jv) *  (un - frac_evap_flood(ji)) * (un - frac_sub_snow(ji)) * rau(ji) * dt_sechiba * &
            (qsol_sat_new(ji)-qair(ji)) / raero(ji)

         ! In case of dew formation, we do not allow a negative transpiration in
         ! plants (an update by leaves directly of air humidiy)
         IF (qsol_sat_new(ji)-qair(ji)<zero .AND. transpir(ji,jv)<zero .AND. ok_hydrol_arch) THEN
            IF (printlev_loc.GT.3) WRITE(numout,*) 'Dew formation ; We don t allow negative transpiration' 
            transpir(ji,jv) = zero
            transpot(ji,jv) = zero
         ENDIF 
         ! for cases when transpir would still be negative - not yet noticed
         IF (transpir(ji,jv)<zero) WRITE(numout,*) 'WARNING : transpir < 0' 
 ...
 !!
 !===========================================================================================================================
  !! SUBROUTINE                 : enerbil_write
  !!
  !!
  !>\BRIEF                      Write a bunch of variables to the history file
  !!
  !! DESCRIPTION                : 
  !! Originally the routine enerbil_main was used for this, but that seemed to
  !! create
  !! problems with uninitialized variables when it was called twice.
  !! \n
  !!
  !! RECENT CHANGE(S): None
  !!
  ...
  SUBROUTINE enerbil_write (kjit, kjpindex, hist_id, hist2_id, index, &
    ...
    REAL(r_std), DIMENSION (kjpindex), INTENT(in)        :: evapot        !! Soil Potential Evaporation (mm/tstep)
    REAL(r_std), DIMENSION (kjpindex), INTENT(in)        :: evapot_corr   !! Soil Potential Evaporation Correction (mm/tstep)
    REAL(r_std), DIMENSION (kjpindex), INTENT(in)        :: fluxsubli     !! Energy of sublimation (mm day^{-1})
    REAL(r_std), DIMENSION (kjpindex), INTENT(in)        :: lwdown        !! LW down radiation by the surface (W m^{-2})
    REAL(r_std), DIMENSION (kjpindex), INTENT(in)        :: lwnet         !! Net Long-wave radiation (W m^{-2})
    REAL(r_std), DIMENSION (kjpindex), INTENT(in)        :: netrad        !! Net radiation (W m^{-2})
    ...
    !! 0.4 Local variables
    REAL(r_std), DIMENSION (kjpindex)                    :: diffevap      !! Difference betwence vevapp and composing 
                                                                          !! fluxes (Kg/m^2/s)
...
!================================================================================================================================
    ...
    CALL xios_orchidee_send_field("lwdown",lwdown)
    ...
    IF ( .NOT. almaoutput ) THEN
       ...
       CALL histwrite_p(hist_id, 'lwdown', kjit, lwdown,  kjpindex, index)
       ...
       IF ( hist2_id > 0 ) THEN
           ...
           CALL histwrite_p(hist2_id, 'lwdown', kjit, lwdown,  kjpindex, index)
