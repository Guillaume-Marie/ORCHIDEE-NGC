MODULE orchoasis_tools
  ...
  INTEGER(i_std), SAVE :: albnir_id, albvis_id, emis_id, z0m_id, z0h_id
  !
...
!
  ...
  !-
  !- orchoasis_wrtgrid
  !- Subroutine to write the grid to OASIS. Only to be called by root-proc
  !-------------------------------------------------------------------------------------------------------
  !
  SUBROUTINE orchoasis_wrtgrid(oasis_gridname, iim, jjm, nbseg, lon, lat, &
       &                       corners, area, mask, river_flag, num_largest_loc)
    !
    ! ARGUMENTS
    !
    CHARACTER(LEN=4), INTENT(in) :: oasis_gridname
    INTEGER(i_std), INTENT(in)   :: iim, jjm, nbseg
    REAL(r_std), INTENT(in) :: lon(iim,jjm), lat(iim,jjm)
    REAL(r_std), INTENT(in) :: corners(iim,jjm,nbseg,2)
    REAL(r_std), INTENT(in) :: area(iim,jjm)
    REAL(r_std), INTENT(in) :: mask(iim,jjm)
    LOGICAL, INTENT(in)     :: river_flag
    INTEGER(i_std), INTENT(in) :: num_largest_loc
    !
    ! LOCAL
    !
    CHARACTER(LEN=4) :: oasis_gridname_tmp
    REAL(r_std), ALLOCATABLE, DIMENSION(:,:,:)  :: corners_lon, corners_lat
    INTEGER(i_std), ALLOCATABLE, DIMENSION(:,:) :: maskinv
    REAL(r_std), ALLOCATABLE, DIMENSION(:,:)    :: lon_estuaries, lat_estuaries
    INTEGER(i_std), ALLOCATABLE, DIMENSION(:,:) :: maskinv_estuaries
    INTEGER(i_std) :: flag
    INTEGER(i_std) :: i, j 
    !
    IF ( .NOT. is_root_prc) THEN
       WRITE(numout,*) "orchoasis_wrtgrid has to be called on the root proc."
       WRITE(numout,*) "We are here on a non root processor. is_root_prc = ", is_root_prc
       WRITE(numout,*) "STOP from orchoasis_wrtgrid"
       STOP
    ENDIF
    !
    CALL oasis_start_grids_writing(flag)
    !
    oasis_gridname_tmp = "LORC"
    CALL oasis_write_grid(oasis_gridname_tmp, iim, jjm, lon, lat)
    !
    ALLOCATE(corners_lon(iim, jjm, nbseg), corners_lat(iim, jjm, nbseg))
    corners_lon(:,:,:) = corners(:,:,:,1)
    corners_lat(:,:,:) = corners(:,:,:,2)
    CALL oasis_write_corner(oasis_gridname_tmp, iim, jjm, nbseg, corners_lon, corners_lat)
    !
    ALLOCATE(maskinv(iim, jjm))
    DO i=1,iim
       DO j=1,jjm
          IF (mask(i,j) == 0) THEN
             maskinv(i,j) = 1
       ...
       ENDDO
    ENDDO
    CALL oasis_write_mask(oasis_gridname_tmp, iim, jjm, maskinv)
    !
    CALL oasis_write_area(oasis_gridname_tmp, iim, jjm, area)
    !
    CALL oasis_terminate_grids_writing()
    !
    IF ( river_flag ) THEN
       ALLOCATE(lon_estuaries(num_largest_loc,1))
       ALLOCATE(lat_estuaries(num_largest_loc,1))
       ALLOCATE(maskinv_estuaries(num_largest_loc,1))
       !
       maskinv_estuaries(:,:) = 0
       oasis_gridname_tmp = "EORC"
       !
!!$       CALL routing_reg_riverlocroot (num_largest_loc, lon_estuaries, lat_estuaries)
       !
       CALL oasis_start_grids_writing(flag)
       CALL oasis_write_grid(oasis_gridname_tmp, num_largest_loc, 1, lon_estuaries, lat_estuaries)
       CALL oasis_write_mask(oasis_gridname_tmp, num_largest_loc, 1, maskinv_estuaries)
       CALL oasis_terminate_grids_writing()
    ENDIF
    !
    DEALLOCATE(corners_lon, corners_lat)
    DEALLOCATE(maskinv)
    IF ( ALLOCATED(lon_estuaries) ) DEALLOCATE(lon_estuaries)
    IF ( ALLOCATED(lat_estuaries) ) DEALLOCATE(lat_estuaries)
    IF ( ALLOCATED(maskinv_estuaries) ) DEALLOCATE(maskinv_estuaries)
    !
  END SUBROUTINE orchoasis_wrtgrid
  !-------------------------------------------------------------------------------------------------------
  !-
  !- orchoasis_defvar
  !-
  ...
  SUBROUTINE orchoasis_defvar (mpi_rank, kjpindex)
    ...
    !
    varname="ROUGHMOM"
    CALL oasis_def_var(z0m_id, varname, il_part_id, var_nodims, OASIS_Out, var_shape, OASIS_Real, ierror)
    tot_error = tot_error+ierror
    !
    varname="ROUGHHEA"
    CALL oasis_def_var(z0h_id, varname, il_part_id, var_nodims, OASIS_Out, var_shape, OASIS_Real, ierror)
    tot_error = tot_error+ierror
    !
  ...
  SUBROUTINE orchoasis_putvar(itau, dt, kjpindex, landindex, vevapp, fluxsens, fluxlat, coastalflow, riverflow, &
       &                      netco2, carblu, tsol_rad, temp_sol_new, qsurf, albedo, emis, z0m, z0h)
    !
    ! ARGUMENTS
    !
    INTEGER(i_std), INTENT(in) :: itau, kjpindex
    ...
    REAL(r_std), DIMENSION(kjpindex), INTENT(in) :: netco2, carblu, tsol_rad, temp_sol_new, qsurf, emis
    REAL(r_std), DIMENSION(kjpindex), INTENT(in) :: z0m, z0h
    ...
    IF ( landonly ) THEN
       buffer(:) = z0m(:)
    ELSE
       buffer(landindex(:)) = z0m(:)
    ENDIF
    CALL oasis_put(z0m_id, NINT(itau*dt), buffer, oasis_info)
    IF ( landonly ) THEN
       buffer(:) = z0h(:)
    ELSE
       buffer(landindex(:)) = z0h(:)
    ENDIF
    CALL oasis_put(z0h_id, NINT(itau*dt), buffer, oasis_info)
    !
...
!
  SUBROUTINE orchoasis_printdate(julian_day, message)
    ...
    seci = INT(sec)
    !
    WRITE(numout,'(I4.4,"-",I2.2,"-",I2.2," ",I2.2,":",I2.2,":",I2.2," > ", A60)') &
