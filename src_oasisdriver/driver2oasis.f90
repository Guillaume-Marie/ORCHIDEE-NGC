PROGRAM driver2oasis
  ...
  CHARACTER(LEN=20) :: calendar, calXIOS
  INTEGER(i_std)    :: nbseg
  !-
  ...
  INTEGER(i_std)                 :: yeardr, monthdr, daydr    !! Time origin date information
  REAL(r_std)                    :: secdr                     !! Time origin date information
!-
!- input fields
!-
  ...
  !- output fields
  !-
  REAL(r_std), ALLOCATABLE, DIMENSION (:)            :: z0m           !! Momentum Surface roughness
  REAL(r_std), ALLOCATABLE, DIMENSION (:)            :: z0h           !! Temperature Surface roughness
  ...
  INTEGER(i_std) :: albnir_id, albvis_id, emis_id, z0m_id, z0h_id
  !
  ...
  !- XIOS
  !-
  LOGICAL                        :: xios_driver_ok = .FALSE.
  TYPE(xios_context)             :: ctx_hdl               !! Handel for driver2oasis
  CHARACTER(len=*),PARAMETER     :: id="driver"           !! Id for initialization of driver2oasis in XIOS
...
!!  CALL getin_name("run.def")
  ...
  !
  xios_driver_ok=.TRUE.
  WRITE(w_unit,*) 'Using XIOS :', xios_driver_ok
  !-
  !---------------------------------------------------------------------------------------
  !-
  ...
  IF ( initmode ) THEN
     ...
     is_root_prc = .TRUE.
     !
     CALL globgrd_getdomsz("NONE", iim_glo, jjm_glo, nbindex_g, model_guess, file_id, forfilename, zoom_lon, zoom_lat)
     !
     CALL forcing_getglogrid(nb_forcefile, forfilename, iim_glo, jjm_glo, nbindex_g, .TRUE.)

     CALL forcing_zoomgrid(zoom_lon, zoom_lat, forfilename(1), model_guess, .TRUE.)
 
     CALL globgrd_writegrid(gridfilename, nbseg)
  ...
  CALL oasis_get_localcomm(TMP_COMM, ierror)
  !
  IF ( xios_driver_ok ) THEN
     CALL xios_initialize(id,local_comm=TMP_COMM, return_comm=LOCAL_OASIS_COMM)
  ELSE
     LOCAL_OASIS_COMM = TMP_COMM
  ENDIF
  !
  CALL Init_orchidee_para(LOCAL_OASIS_COMM, .FALSE.)
  !-
  !-
  !---------------------------------------------------------------------------------------
  !-
  ...
  ALLOCATE(z0m(nbindex_g))
  ALLOCATE(z0h(nbindex_g))
  !
  !-
  !---------------------------------------------------------------------------------------
  !-
  ...
  ! 
  varname="Z0MODRIV"
  CALL oasis_def_var(z0m_id, varname, il_part_id, var_nodims, OASIS_In, var_shape, OASIS_Real, ierror)
  !
  varname="Z0HEDRIV"
  CALL oasis_def_var(z0h_id, varname, il_part_id, var_nodims, OASIS_In, var_shape, OASIS_Real, ierror)
  !
  CALL oasis_enddef (ierror)
  !-
  WRITE(w_unit,*) 'OASIS Vars declared'
  !
  !---------------------------------------------------------------------------------------
  !-
  !- Set-up XIOS
  ...
  !
  IF ( xios_driver_ok ) THEN
     CALL xios_context_initialize("driver2oasis", LOCAL_OASIS_COMM)
     CALL xios_get_handle("driver2oasis",ctx_hdl)
     CALL xios_set_current_context(ctx_hdl)
     !
     CALL ju2ymds(date_start, yeardr, monthdr, daydr, secdr)
     !
     dtime_xios%second=dt
     IF (INDEX(calendar, 'gregor') > 0 .OR. INDEX(calendar, 'Gregor') > 0 .OR. &
          &    calendar == 'standard') THEN
        calXIOS = 'gregorian'
     ELSE IF (calendar == 'noleap') THEN
        calXIOS = 'noleap'
     ELSE IF (calendar == '360d') THEN
        calXIOS = 'd360'
     END IF
     
     CALL xios_define_calendar(type=calXIOS, start_date=xios_date(yeardr,monthdr,daydr,0,0,0), &
          time_origin=xios_date(yeardr,monthdr,daydr,0,0,0), timestep=dtime_xios)
     CALL xios_set_domain_attr("domain_landpoints", ni_glo=iim_glo, nj_glo=jjm_glo, type="curvilinear")
     CALL xios_set_domain_attr("domain_landpoints",lonvalue_2d=lon_glo,latvalue_2d=lat_glo)
     CALL xios_close_context_definition()
  ENDIF
  WRITE(w_unit,*) 'XIOS Closed Context'
  !
  !---------------------------------------------------------------------------------------
  !-
  ...
  DO itau = 0,nbdt-1
     ...
     !
     CALL xios_set_current_context(ctx_hdl)
     CALL xios_update_calendar(itau+1)
     !
     ...
     CALL oasis_get(z0m_id, NINT((itau)*dt), z0m, oasis_info)
     CALL oasis_get(z0h_id, NINT((itau)*dt), z0h, oasis_info)
     ...
     IF ( MOD(itau, freq_diag) == 0 ) THEN
        ...
        WRITE(w_unit,*) MINVAL(z0m), " << Z0M << ", MAXVAL(z0m)
        !
  ...
  !-
  CALL forcing_close()
  WRITE(w_unit,*) "Closed forcing"
  !-
  IF ( xios_driver_ok ) THEN
     CALL xios_context_finalize()
     CALL xios_finalize()
     WRITE(w_unit,*) "XIOS finalized"
     CALL oasis_terminate(ierror)
  ELSE
     CALL oasis_terminate(ierror)
  ENDIF
  WRITE(w_unit,*) "OASIS closed"
  !-
  CLOSE(UNIT=w_unit)
  !-
