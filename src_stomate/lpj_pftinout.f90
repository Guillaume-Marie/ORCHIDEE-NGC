...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_stomate/lpj_pftinout.f90 $
...
!_ ================================================================================================================================
  ...
  SUBROUTINE pftinout (npts, dt, adapted, regenerate, bm_sapl_2D, &
  ...
  !! 0. Variable and parameter declaration
    ...
    REAL(r_std),DIMENSION(npts,nvm,nparts,nelements), INTENT(in) :: bm_sapl_2D   !! Spatialized biomass of sapling
    ...
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)           :: RIP_time        !! How much time ago was the PFT eliminated for 
                                                                                 !! the last time (years)    
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)           :: co2_to_bm       !! C biomass uptaken (gC m^{-2} day^{-1})   
    REAL(r_std), DIMENSION(npts,nvm), INTENT(inout)           :: n_to_bm       !! N biomass uptaken (gN m^{-2} day^{-1})   
       ...
       IF ( natural(j).AND.is_tree(j) ) THEN
          WHERE(fracnat(:).GT.min_stomate) 
                sumfrac_wood(:) = sumfrac_wood(:) + cn_ind(:,j) * ind(:,j) / fracnat(:) & 
                     * ( un - exp( - biomass_to_lai(lm_lastyearmax(:,j),npts,j) * ext_coeff(j) ) )
          ...
          ELSE
             ...
             DO i = 1, npts ! Loop over # pixels - domain size
                ...
                IF ( ( veget_cov_max(i,j) .GT. min_stomate ) .AND. ( .NOT. PFTpresent(i,j) ) ) THEN
                   ...
                   biomass(i,j,:,:) = bm_sapl_2D(i,j,:,:) * ind(i,j) /veget_cov_max(i,j) 
  ...
  !! 6. Introduce PFTs
    ...
    DO j = 2,nvm ! Loop over # PFTs
       ...
       IF ( natural(j) ) THEN
          ...
          IF ( NbNeighb /= 8 ) THEN
             CALL ipslerr_p(3, "pftinout", "This routine needs to be adapted to non rectengular grids", &
                  &           "Talk to Jan Polcher", " ")
          ENDIF
          ...
          WHERE ( can_introduce(:) )
             ...
             WHERE(veget_cov_max(:,j) .GT. min_stomate)
                biomass(:,j,ileaf,icarbon) = bm_sapl_2D(:,j,ileaf,icarbon) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapabove,icarbon) = bm_sapl_2D(:,j,isapabove,icarbon) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapbelow,icarbon) = bm_sapl_2D(:,j,isapbelow,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartabove,icarbon) = bm_sapl_2D(:,j,iheartabove,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartbelow,icarbon) = bm_sapl_2D(:,j,iheartbelow,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iroot,icarbon) = bm_sapl_2D(:,j,iroot,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ifruit,icarbon) = bm_sapl_2D(:,j,ifruit,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,icarbres,icarbon) = bm_sapl_2D(:,j,icarbres,icarbon) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ilabile,icarbon) = bm_sapl_2D(:,j,ilabile,icarbon) * ind(:,j)/veget_cov_max(:,j)

                biomass(:,j,ileaf,initrogen) = bm_sapl_2D(:,j,ileaf,initrogen) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapabove,initrogen) = bm_sapl_2D(:,j,isapabove,initrogen) * ind(:,j) /veget_cov_max(:,j)
                biomass(:,j,isapbelow,initrogen) = bm_sapl_2D(:,j,isapbelow,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartabove,initrogen) = bm_sapl_2D(:,j,iheartabove,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iheartbelow,initrogen) = bm_sapl_2D(:,j,iheartbelow,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,iroot,initrogen) = bm_sapl_2D(:,j,iroot,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ifruit,initrogen) = bm_sapl_2D(:,j,ifruit,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,icarbres,initrogen) = bm_sapl_2D(:,j,icarbres,initrogen) * ind(:,j)/veget_cov_max(:,j)
                biomass(:,j,ilabile,initrogen) = bm_sapl_2D(:,j,ilabile,initrogen) * ind(:,j)/veget_cov_max(:,j)
             ELSEWHERE              
                biomass(:,j,ileaf,icarbon) = bm_sapl_2D(:,j,ileaf,icarbon) * ind(:,j)
                biomass(:,j,isapabove,icarbon) = bm_sapl_2D(:,j,isapabove,icarbon) * ind(:,j)
                biomass(:,j,isapbelow,icarbon) = bm_sapl_2D(:,j,isapbelow,icarbon) * ind(:,j)
                biomass(:,j,iheartabove,icarbon) = bm_sapl_2D(:,j,iheartabove,icarbon) * ind(:,j)
                biomass(:,j,iheartbelow,icarbon) = bm_sapl_2D(:,j,iheartbelow,icarbon) * ind(:,j)
                biomass(:,j,iroot,icarbon) = bm_sapl_2D(:,j,iroot,icarbon) * ind(:,j)
                biomass(:,j,ifruit,icarbon) = bm_sapl_2D(:,j,ifruit,icarbon) * ind(:,j)
                biomass(:,j,icarbres,icarbon) = bm_sapl_2D(:,j,icarbres,icarbon) * ind(:,j)
                biomass(:,j,ilabile,icarbon) = bm_sapl_2D(:,j,ilabile,icarbon) * ind(:,j) 

                biomass(:,j,ileaf,initrogen) = bm_sapl_2D(:,j,ileaf,initrogen) * ind(:,j)
                biomass(:,j,isapabove,initrogen) = bm_sapl_2D(:,j,isapabove,initrogen) * ind(:,j)
                biomass(:,j,isapbelow,initrogen) = bm_sapl_2D(:,j,isapbelow,initrogen) * ind(:,j)
                biomass(:,j,iheartabove,initrogen) = bm_sapl_2D(:,j,iheartabove,initrogen) * ind(:,j)
                biomass(:,j,iheartbelow,initrogen) = bm_sapl_2D(:,j,iheartbelow,initrogen) * ind(:,j)
                biomass(:,j,iroot,initrogen) = bm_sapl_2D(:,j,iroot,initrogen) * ind(:,j)
                biomass(:,j,ifruit,initrogen) = bm_sapl_2D(:,j,ifruit,initrogen) * ind(:,j)
                biomass(:,j,icarbres,initrogen) = bm_sapl_2D(:,j,icarbres,initrogen) * ind(:,j)
                biomass(:,j,ilabile,initrogen) = bm_sapl_2D(:,j,ilabile,initrogen) * ind(:,j) 
             END WHERE
             ...
             lm_lastyearmax(:,j) = bm_sapl_2D(:,j,ileaf,icarbon) * ind(:,j)
