...
!! 
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_stomate/lpj_gap.f90 $
!! $Date: 2024-09-23 14:55:49 +0200 (lun. 23 sept. 2024) $
!! $Revision: 8671 $
!! \n
!_ ================================================================================================================================
...
!_ ================================================================================================================================
  ...
  SUBROUTINE gap (npts, dt, &
    ...
    INTEGER(i_std)                                          :: ipts,ipft,ipar,iele     !! Indices
...
!_ ================================================================================================================================
    ...
    ! +++ CHECK +++ 
    ! indexes should be renamed, mortality also (in tree_mortality, availability
    ! also as suggested above, etc.
    ! +++++++++++++
    ! loop over #PFT
    DO ipft = 2,nvm
 ...
 !! 1. Tree mortality
       ...
       IF ( is_tree(ipft) ) THEN 
          ...
          IF ( .NOT.  lpj_gap_const_mort  ) THEN
             ...
             WHERE ( PFTpresent(:,ipft) .AND. ( lm_lastyearmax(:,ipft) .GT. min_stomate ) )
            ...
            ! calculate net biomass increment
             delta_biomass(:) = MAX( npp_longterm(:,ipft) - ( turnover_longterm(:,ipft,ileaf,icarbon) + &
                  turnover_longterm(:,ipft,iroot,icarbon) + turnover_longterm(:,ipft,ifruit,icarbon) + & 
                  turnover_longterm(:,ipft,isapabove,icarbon) + turnover_longterm(:,ipft,isapbelow,icarbon) ) ,zero)
            ...
            !        maximum LAI. (corresponding actually to the maximum LAI of the previous year)
             ...
             vigour(:) = delta_biomass(:) / (biomass_to_lai(lm_lastyearmax(:,ipft),npts,ipft))
             ...
             WHERE ( PFTpresent(:,ipft) )
                
                availability(:) = availability_fact(ipft) / ( un + ref_greff * vigour(:) )
                ! Scale mortality by timesteps per year
                mortality(:,ipft) = MAX(min_avail,availability(:))  * dt/one_year  
          ...
          ELSE  ! .NOT. lpj_gap_const_mort
             ...
             WHERE ( PFTpresent(:,ipft) )
                ...
                mortality(:,ipft) = dt/(residence_time(ipft)*one_year)
          ...
          IF ( ok_dgvm ) THEN
             ...
             WHERE ( PFTpresent(:,ipft) .AND. ( npp_longterm(:,ipft) .LE. min_stomate ) )
          ...
          IF ( ok_dgvm .AND. (tmin_crit(ipft) .NE. undef) ) THEN
             ! frost-sensitive PFTs
             WHERE ( t2m_min_daily(:) .LT. tmin_crit(ipft) )
                mortality(:,ipft) = MIN(un,(coldness_mort*(tmin_crit(ipft)-t2m_min_daily(:))+mortality(:,ipft) ) )
             ENDWHERE
          ENDIF

          IF ( ok_dgvm .AND. leaf_tab(ipft)==1 .AND. pheno_type(ipft)==2) THEN
             ! Treat the spring frost for broadleaf and summergreen vegetations
             ! leaf_tab=broadleaf and pheno_typ=summergreen
             DO ipts=1,npts
                IF ( (Tmin_spring_time(ipts,ipft)>0) .AND. (Tmin_spring_time(ipts,ipft)<spring_days_max+1) ) THEN
                   IF ( t2m_min_daily(ipts) .LT. frost_damage_limit ) THEN
                      mortality(ipts,ipft) = MIN(un,(0.01*(frost_damage_limit-t2m_min_daily(ipts))* &
                           Tmin_spring_time(ipts,ipft)/spring_days_max+mortality(ipts,ipft) ) )
                   END IF
                END IF
             END DO
          END IF
          ...
          DO iele = 1,nelements
             DO ipar = 1, nparts
                ...
                WHERE ( PFTpresent(:,ipft) )
                   ...
                   dmortality(:) =  mortality(:,ipft) * biomass(:,ipft,ipar,iele)
                   bm_to_litter(:,ipft,ipar,iele) = bm_to_litter(:,ipft,ipar,iele) + dmortality(:)
                   biomass(:,ipft,ipar,iele) = biomass(:,ipft,ipar,iele) - dmortality(:)
          ...
          IF ( ok_dgvm ) THEN
             ...
             WHERE ( PFTpresent(:,ipft) )
                ...
                ind(:,ipft) = ind(:,ipft) * ( un - mortality(:,ipft) )
 ...
 !! 2. Grasses mortality
          ...
          IF ( .NOT.ok_dgvm .AND. .NOT.lpj_gap_const_mort) THEN
             ...
             WHERE ( PFTpresent(:,ipft) .AND. ( npp_longterm(:,ipft) .LE. npp_longterm_init ) )
 ...
 !! 3. Write to history files
    ...
    CALL histwrite_p (hist_id_stomate, 'TREE_MORTALITY', itime, &
