...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_global/solar.f90 $
!! $Date: 2025-02-06 15:55:07 +0100 (jeu. 06 févr. 2025) $
!! $Revision: 8817 $
!! \n
!_ ================================================================================================================================
...
!! ================================================================================================================================
!! SUBROUTINE   : solarang_noon
!!
!>\BRIEF         This subroutine computes the solar angle according at noon and is based on solarang
!!
!! DESCRIPTION  : None
!!
!! RECENT CHANGE(S): None
!!
!! MAIN OUTPUT VARIABLE(S): ::coszang_noon
!!
!! REFERENCE(S) : See for further details :
!! http://www.atmo.arizona.edu/~morrill/swrad.html
!!
...
!! \n
!_ ================================================================================================================================
  SUBROUTINE solarang_noon(kjpindex, lalo, coszang_noon)
    
    Use time, ONLY: julian_diff, one_year
    IMPLICIT NONE
    ...
    REAL(r_std), DIMENSION(kjpindex), INTENT(out)   :: coszang_noon       !! solar zenith angle at noon

    !! 0.4 Local variables
    REAL(r_std)                                     :: gamma              !! Day angle (radians)
    REAL(r_std)                                     :: dec                !! Declination angle (radians)
    REAL(r_std), DIMENSION (kjpindex)               :: llat               !! Latitudal angle (radians)

    !_ ================================================================================================================================

       ! Day angle gamma
       gamma = 2.*pi*(julian_diff)/one_year
       ! Solar declination (assumed constant for a 24 hour period) in radians
       dec = ( 0.006918-0.399912*COS(gamma)+0.070257*SIN(gamma) &
         &       -0.006758*COS(2*gamma)+0.000907*SIN(2*gamma)      &
         &       -0.002697*COS(3*gamma)+0.00148*SIN(3*gamma))
       ! latitude
       ...
       coszang_noon(:) = SIN(dec)*SIN(llat)+COS(dec)*COS(llat)
       WHERE (coszang_noon .LT. zero)
          coszang_noon(:) = zero
       ENDWHERE
    
  END SUBROUTINE solarang_noon
...
!_ ================================================================================================================================
  ...
  !! ================================================================================================================================
!! SUBROUTINE   : solarang_unstruct
!!
!>\BRIEF         This subroutine computes the solar angle according to the method used by GSWP and developed by J.C. Morill. Modified for unconstructured grids.
!!
!! DESCRIPTION  : None
!!
!! RECENT CHANGE(S): None
...
!_ ================================================================================================================================
  ...
  SUBROUTINE solarang_unstruct (julian, julian0, iim, jjm, lon, lat, csang)
...
!_ ================================================================================================================================
    ...
    !-
    gmt = 24.*(julian-INT(julian))
    IF (.NOT.ALLOCATED(zone))  ALLOCATE(zone(jjm))
    IF (.NOT.ALLOCATED(lhour)) ALLOCATE(lhour(jjm))
    !-
    !igmt = NINT(gmt)
    IF (printlev >= 4) WRITE(numout,*) 'Get time zone', et, gmt
    CALL time_zone_unstruct(gmt, iim, jjm, lon, zone, lhour)
    !- Start a loop through the grid
    !-
    IF (printlev >= 4) WRITE(numout,*) 'Start a loop through the grid'
    DO ilon=1,jjm
    ...
    ENDDO
    !----------------------
  END SUBROUTINE solarang_unstruct


  !! ================================================================================================================================
!! SUBROUTINE   : time_zone_unstruct
!!
!>\BRIEF         
!!
!! DESCRIPTION  : None
...
!_ ================================================================================================================================
  ...
  SUBROUTINE time_zone_unstruct (gmt, iim, jjm, lon, zone, lhour)
    ...
    INTEGER, DIMENSION(jjm), INTENT(out)  :: zone            !!
    REAL, DIMENSION(jjm), INTENT(out)     :: lhour           !!
...
!_ ================================================================================================================================
    ...
    ENDDO
    !-----------------------
  END SUBROUTINE time_zone_unstruct
