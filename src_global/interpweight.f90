...
!!
!! SVN         :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_global/interpweight.f90 $
!! $Date: 2025-05-23 16:56:41 +0200 (ven. 23 mai 2025) $
!! $Revision: 8970 $
!! \n
!_================================================================================================================================
  ...
  PUBLIC interpweight_1D, interpweight_2D, interpweight_3D, interpweight_4D, interpweight_2Dcont,     &
    ...
    interpweight_Index2DLonLat, interpweight_ValVecR, interpweight_3D_lake
...
! Scaling to 1 the quality variable
    DO ip=1, nbpt
       IF (contfrac(ip) == zero) THEN
          ! HACK: This should not happen but it does sometimes.
          ! We can't calculate aoutvar, therefor assign a value -2
          aoutvar(ip) = -2
       ELSE
...
! Test lecture
    DO ip=1,lml
      IF ( SUM(invar3D(:,:,ip)) < 0.00001 ) THEN
        ...
        msg = 'All values are zero or smaller than 0.00001 for data for type ' // TRIM(iS)
        CALL ipslerr(1,'interpweight_3D',TRIM(msg),'','')
      END IF
    END DO
...
!! ================================================================================================================================
!! SUBROUTINE   : interpweight_3D_lake
!!
!>\BRIEF        ! Interpolate input files needed for lake.f90 to the grid of the model for a 3D-variable  
!!
!! DESCRIPTION  : (definitions, functional, design, flags): 
!!
!! RECENT CHANGE(S): None
...
!! \n
!_ ================================================================================================================================
  SUBROUTINE interpweight_3D_lake(nbpt, Nvariabletypes, variabletypes, lalo, resolution, neighbours,       &
       contfrac, filename, varname, inlonname, inlatname, varmin, varmax, noneg, masktype,               &
       maskvalues, maskvarname, dim1, dim2, initime, typefrac,                                           &
       maxresollon, maxresollat, outvar3D, aoutvar)
...
!_ ================================================================================================================================
    ...
    IF (printlev >= 1) WRITE(numout, *)"In interpweight_3D_lake filename: ",TRIM(filename),         &
         " varname:'" // TRIM(varname) // "'"
! Allocating input data
    IF (is_root_prc) THEN
       IF (printlev >=1) THEN
          WRITE(numout,*) "Entering 'interpweight_3D_lake'. Debug mode."
          WRITE(numout,*)'(/," --> fliodmpf")'
          CALL fliodmpf (TRIM(filename))
          WRITE(numout,*)'(/," --> flioopfd")'
       ENDIF
    ...
    CASE (2)
       ...
       ALLOCATE(invar2D(iml,jml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake',"Problem in allocation of variable '" //     &
            TRIM(varname) ,'','')
       IF (is_root_prc) CALL flinget(fid, TRIM(varname), iml, jml, 0, 0, 1, 1, invar2D)
       IF (printlev >= 1) THEN
          WRITE(numout,*)'  interpweight_3D_lake input data dimensions'
          WRITE(numout,*)'    iml, jml:',iml, jml
       END IF
       
    CASE (3)
       ALLOCATE(indims(3), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of indims','','')
       indims = interpweight_get_var3dims_file(filename, varname)
    ...
    CASE (4)
       ...
       IF (initime /= -1) THEN
          ...
          IF (printlev >= 1) WRITE (numout,*) &
               'interpweight_3D_lake: taking into account the initial reading time:',initime
          IF (initime <= 0) tml = 0
          
          CALL bcast(tml)
          ALLOCATE(invar3D(iml,jml,lml), STAT=ALLOC_ERR)
          IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable invar3D','','')
          IF ( initime > 0 ) THEN
             IF (is_root_prc) THEN
                IF (initime <= tml) THEN
                   CALL flinget(fid, TRIM(varname), iml, jml, lml, 1, initime, 1, invar3D)
       ...
       ELSE
          CALL bcast(tml)
          ALLOCATE(invar4D(iml,jml,lml,tml), STAT=ALLOC_ERR)
          IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable invar4D','','')
          IF (is_root_prc) CALL flinget(fid, TRIM(varname), iml, jml, lml, tml, 1, tml, invar4D)
       END IF
    ...
    CASE DEFAULT
       WRITE (S1,'(I3)') inNdims
       CALL ipslerr_p(3,'interpweight_3D_lake',"Input number of dimensions: '" // TRIM(S1) // "' not ready !!", &
            '','')
    ...
    IF (printlev >= 1) WRITE(numout,*)'  interpweight_3D_lake getting variable ended!'
    ! Test lecture
    DO ip=1,lml
       IF ( SUM(invar3D(:,:,ip)) < 0.00001 ) THEN
          write(numout,*)'SUM(invar3D(ip))=',SUM(invar3D(:,:,ip)),' for ip=',ip
          CALL ipslerr(2,'interpweight_3D_lake','SUM(invar3D)< 0.00001 ','JG changed from stop to warning','')
       END IF
    END DO
    ...
    IF (inNLldims == 1) THEN
       ! Allocate memory for latitudes
       ALLOCATE(lon_in(iml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable lon_in','','')
       
       ALLOCATE(lat_in(jml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable lat_in','','')
       ...
       ALLOCATE(lon(iml,jml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable lon','','')
       ALLOCATE(lat(iml,jml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable lat','','')
    ...
    ELSE
       ! Allocate memory for longitude
       ALLOCATE(lon(iml,jml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Pb in allocation for lon','','')
       ! Allocate memory for latitudes
       ALLOCATE(lat(iml,jml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Pb in allocation for lat','','')
    ...
    ALLOCATE(resol_in(iml,jml,2), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable resol_in','','')
    ...
    IF (printlev >= 1) WRITE(numout,*)'  interpweight_3D_lake modifying input ended!'
    
    ALLOCATE(mask(iml,jml), STAT=ALLOC_ERR)
    IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable mask','','')
    mask(:,:) = 0
     
    IF (TRIM(masktype) == 'var') THEN
       ALLOCATE(maskvar(iml,jml), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable maskvar','','')
       maskvar(:,:) = 0.
       ! Reads the mask from the file
       IF (is_root_prc) THEN
          ! ATTTENTION!! One can not use flinget in this way. It always spect to have a REAL variable!
    ...
    IF (printlev >= 1) WRITE(numout,*)'  interpweight_3D_lake masking input ended!'
    ...
    ok_interpol = .FALSE.
    DO WHILE ( .NOT. ok_interpol )
       IF (printlev>=1) WRITE(numout,*) "Projection arrays for ",callsign," : "
       IF (printlev>=1) WRITE(numout,*) "nbvmax = ",nbvmax, nix, njx
       
       ALLOCATE(sub_index(nbpt, nbvmax, 2), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable sub_index','','')
       
       sub_index(:,:,:)=zero
       
       ALLOCATE(sub_area(nbpt, nbvmax), STAT=ALLOC_ERR)
       IF (ALLOC_ERR /= 0) CALL ipslerr_p(3,'interpweight_3D_lake','Problem in allocation of variable sub_area','','')
       
       sub_area(:,:)=zero
       
       IF (printlev >= 1) THEN
          WRITE(numout,*) 'interpweight_3D_lake:'
          WRITE(numout,*) '  Carteveg range LON:', MINVAL(lon), MAXVAL(lon)
          WRITE(numout,*) '  Carteveg range LAT:', MINVAL(lat), MAXVAL(lat)
       END IF
    ...
    ! Part inspirated from interpweight_provide_fractions3D
    IF (Nvariabletypes /= lml) THEN
    ...
    IF (ALL(invar3D == zero)) THEN
       msg = "' wrong 3D input variable"
       CALL ipslerr_p(3,'interpweight_provide_fractions3D',"'" // TRIM(msg),'','')
    END IF
    
    DO ib=1,nbpt
       DO it = 1, Nvariabletypes
          outvar3D(ib,it) = invar3D(sub_index(ib,1,1),sub_index(ib,1,2),it)
       END DO
    END DO
    
    ! aoutvar is normally an output from interpweight_provide_fractions3D
    ! here it is not calculate. Set 1 for all points as if the interpolation was successfull. 
    aoutvar(:)=1
    
    IF (printlev >= 1) WRITE(numout,*)'interpweight_3D_lake end of interpweight_provide_fractions3D'
    
    IF (printlev>=1) THEN
       IF (COUNT(aoutvar == -1) >= AINT(nbpt*0.4)) THEN
          WRITE(numout,*) 'interpweight_3D_lake total number of points: ', nbpt
          WRITE(numout,*) '  Interpolated: ', COUNT(aoutvar /= -1),                 &
               ' Non-interpolated: ', COUNT(aoutvar == -1)
          msg = "Less than 60% of points '" // TRIM(filename) // "' variable '" // TRIM(varname) //       & 
               "' have been succesfully interpolated on the current process !!"
       ...
       ELSE
          msg = TRIM(filename) // "' variable: '" // TRIM(varname) // "' interpolation Done"
          WRITE(numout,*) "interpweight_3D_lake: '" // TRIM(msg)
          WRITE(numout,*) '  Total number of points: ', nbpt
          WRITE(numout,*) '  Interpolated: ', COUNT(aoutvar /= -1),                 &
               ' Non-interpolated: ', COUNT(aoutvar == -1)
       END IF
  ...
  END SUBROUTINE interpweight_3D_lake
...
! Providing the interpolated grid point
    ...
    DO ip=1, nbpt
       ...
       !+++HACK+++
       IF (aoutvar(ip) /= -1) THEN
          IF (contfrac(ip) == zero) THEN
             ! HACK: This should not happen but it does sometimes.
             ! We can't calculate aoutvar, therefor assign a value -2
             aoutvar(ip) = -2
          ELSE
             ! The original code assumes that for every pixels that needs
             ! to be considered by ORCHIDEE the land-sea mask has a value
             ! This is a fair assumption. This assumption should be checked
             ! earlier on in the code. This is the original code.
             aoutvar(ip) = aoutvar(ip)/(resolution(ip,1)*resolution(ip,2))/contfrac(ip)
          END IF
       END IF
       !++++++++++++
    END DO
