...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_global/time.f90 $
!! $Date: 2025-05-27 12:57:41 +0200 (mar. 27 mai 2025) $
!! $Revision: 8973 $
!! \n
!_ ================================================================================================================================
MODULE time
...
!$OMP THREADPRIVATE(LastTsDay)
  LOGICAL, PUBLIC                     :: FirstDayYear       !! Flag is true during the whole time step for the first day of the year
!$OMP THREADPRIVATE(FirstDayYear)
  LOGICAL, PUBLIC                     :: annual_proc_end_of_year !! Flag to determine when the annual processes should be done
!$OMP THREADPRIVATE(annual_proc_end_of_year)
  LOGICAL, PUBLIC                     :: ts_annual_proc     !! Flag is true when it is the time-step for the annual processes to be done.
                                                            !! By default this equals to last time-step of the year but it can be changed from run.def
!$OMP THREADPRIVATE(ts_annual_proc)
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE time_initialize(kjit, date0_loc, dt_sechiba_loc, tstepint)
    ...
    printlev_loc=get_printlev("time")
    ...
    !! Send a warning if the time step is longer than one hour
    IF (dt_sechiba > 3600) THEN
       CALL ipslerr_p(2,'time_initialize', 'The time step, DT_SECHIBA, is longer than 1 hour.',&
         'This has not been validated and might create an error in the water buget closure.',&
         'See also ticket https://forge.ipsl.fr/orchidee/ticket/953')
    END IF

    !Config Key   = ANNUAL_PROC_END_OF_YEAR
    !Config Desc  = If true the annual processes are done at the last time-step of the year(recommanded), otherwise it is done the last time-step of June
    !Config If    = OK_STOMATE
    !Config Def   = y
    !Config Help  = Setting this option to false is only done to test restartability for annual processes in stomate
    !Config Units = [flag]
    annual_proc_end_of_year=.TRUE.
    CALL getin_p('ANNUAL_PROC_END_OF_YEAR', annual_proc_end_of_year)
...
!_ ==============================================================================================================================
  SUBROUTINE time_nextstep(kjit)
    ...
    LOGICAL                      :: LastTsJune !! Last time step in June

    !! Calculate the new time interval for current time step
    CALL time_interval(kjit, year_start, month_start, day_start, sec_start, julian_start, &
                             year_end,   month_end,   day_end,   sec_end,   julian_end)
    
    !! Calculate julian0: the julian date for the 1st of January in current year
    IF ( kjit == 0 ) then
       CALL ymds2ju(year_end, 1, 1, zero, julian0)
    ELSE
    ...
    ELSE IF (sec_start >= one_day-dt_sechiba-1 .AND. sec_start < one_day-1 ) THEN
       ...
       IF ( day_start == month_len ) THEN
          ...
          IF ( month_start == 12) THEN
             LastTsYear = .TRUE.
          ELSEIF ( month_start == 6) THEN
             LastTsJune = .TRUE.
          END IF
       END IF
    END IF

    IF (day_start ==1 .AND. month_start ==1) THEN
       FirstDayYear = .TRUE.
    END IF

    !! Set ts_annual_proc
    IF (annual_proc_end_of_year) THEN
       ! By default, the last time-step of the year is set
       ts_annual_proc=LastTsYear
    ELSE
       ! Else, the last time-step of June is set
       ts_annual_proc=LastTsJune
    END IF
    ...
    IF (printlev_loc >= 4) THEN
       ...
       WRITE(numout,*) "time_nextstep: ts_annual_proc=",ts_annual_proc
       WRITE(numout,*) ""
    ELSEIF ((printlev_loc >= 2) .AND. ts_annual_proc) THEN
       WRITE(numout,*) "time_nextstep: ts_annual_proc=TRUE"
    END IF
...
!_ ==============================================================================================================================
  ...
  SUBROUTINE time_interval(kjit, year_s, month_s, day_s, sec_s, julian_s, &
                                 year_e, month_e, day_e, sec_e, julian_e)
    ...
    REAL(r_std), INTENT(out)                    :: sec_s, julian_s
    REAL(r_std), INTENT(out)                    :: sec_e, julian_e
    ...
    SELECT CASE (TRIM(tstepint_type))
       CASE ("START ")
          ...
          julian_s = itau2date(kjit, date0_save, dt_sechiba)
          !! Calculate the end date of the interval using kjti+1
          julian_e = itau2date(kjit+1, date0_save, dt_sechiba)
       CASE("END")
          !! Calculate the start date of the interval
          julian_s = itau2date(kjit-1, date0_save, dt_sechiba)
          !! Calculate the end date of the interval
          julian_e = itau2date(kjit, date0_save, dt_sechiba)
       CASE DEFAULT
       ...
       !! Calculate year, month, day and sec for julian_s date
       CALL ju2ymds (julian_s, year_s, month_s, day_s, sec_s)

       !! Calculate year, month, day and sec for julian_e date
       CALL ju2ymds (julian_e, year_e, month_e, day_e, sec_e)
