...
!!
!! SVN          :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_parameters/vertical_soil.f90 $
!! $Date: 2021-04-07 20:52:57 +0200 (mer. 07 avril 2021) $
!! $Revision: 7149 $
!! \n
!_ ================================================================================================================================
MODULE vertical_soil
...
!! zdr = depth of the layers following Derosnay 1999 (PhD, figure C.2 page 156)
!! layers are "centered" at the nodes (znh). Interfaces between layers
!! are at znh(i)-dnh(i)/2. Note that we need 12 values to describe the
!  11 layers.
!! [ 0.00000000  0.00097752  0.00391007  0.00977517  0.02150538  0.04496579
!!   0.09188661  0.18572825  0.37341154  0.74877810  1.49951124  2.00000000 ]
!!
...
!_ ================================================================================================================================
  ...
  SUBROUTINE vertical_soil_init
    ...
    REAL(r_std)                          :: dis_80               !! distance to 80cm below ground 
    REAL(r_std)                          :: dis_tmp              !! Temporary variable 
    !_ ================================================================================================================================ 
    ...
    IF (ier /= 0) CALL ipslerr_p(3,'vertical_soil_init','Problem in allocate of variable zdr','','')
    !
    ...
    !
    ! Calculate the layers follow Derosnay 1999
    !
    DO i = 1,nslm
       zdr(i-1) = znh(i)-dnh(i)/2
    ENDDO

    ! Add the depth of the lowest layer.
    zdr(nslm) = znh(nslm)
    ...
    ! Find the index of the layer which bottom is the closest to 
    ! 80 cm below ground. This layer is used to calculate whether 
    ! the soil is frozen at 80 cm depth and to calculate whether a
    ! tree is deep or shallow rooting. Both, freezing and rooting 
    ! depth affect the storm damage. Because we are interested in 
    ! the depth of bottom of the layer we can use ::zdr in this
    ! calculation.
    dis_80 = large_value 
    dis_tmp= large_value 
    DO i=1,nslm 
       dis_80 = ABS( 0.8 - zdr(i)) 
       IF ( dis_80 .LT. dis_tmp) THEN 
          dis_tmp = dis_80 
          grnd_80 = i 
       ENDIF
    ENDDO
    ...
    IF (printlev >=1) THEN
       ...
       WRITE(numout,*) "Layer closest to 80 cm depth:",grnd_80, "depth(grnd_80):",zlt(grnd_80)
       WRITE(numout,*) "============================================================================="
    END IF
