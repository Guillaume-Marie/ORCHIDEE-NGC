...
!!
!! SVN              :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_parallel/ioipsl_para.f90 $ 
!! $Date: 2023-07-04 16:53:02 +0200 (mar. 04 juil. 2023) $
!! $Revision: 8074 $
!! \n
!_ ================================================================================================================================
...
!$OMP THREADPRIVATE( orch_ipslout, orch_ilv_cur, orch_ilv_max )
  ...
  PRIVATE :: restransfer_opp_r1d, restransfer_opp_r2d, restransfer_opp_r3d, &
                restransfer_opp_r4d, restransfer_opp_r5d, restransfer_scal, &
             ioget_var_is_scalar, rest_o_nb_dims
...
!-
 ...
 INTERFACE restget_p
  ...
  INTERFACE restransfer 
     MODULE PROCEDURE restransfer_scal, restransfer_var
  END INTERFACE
...
CONTAINS
  ...
  SUBROUTINE ipslerr_p_str (plev,pcname,pstr1,pstr2,pstr3)
    ...
    IF (plev == 3) THEN
       ...
       ! Force to print text output using FLUSH only if cpp flag CPP_FLUSH is set in arch-XXX.fcm
#ifdef CPP_FLUSH
...
! DO NOT USE THIS FUNCTION WITH NON GRID VARIABLE !
   ...
   IF (is_root_prc) THEN 
    ...
    IF (grid_type==unstructured) THEN
       ...
       ENDIF
      CALL gather_unindexed(ind_cell_glo,ind_cell_glo_glo)
      IF (is_root_prc)  CALL restget (fid, vname_q, iim, jjm, llm, zzm, itau, def_beha, &
                                      temp_g, MY_OPERATOR, nbindex, ind_cell_glo_glo(ijndex(:)))
...
! DO NOT USE THIS FUNCTION WITH NON GRID VARIABLE !
    ...
    IF (grid_type==unstructured) THEN
       ...
       ENDIF
      CALL gather_unindexed(ind_cell_glo,ind_cell_glo_glo)
      IF (is_root_prc)  CALL restget (fid, vname_q, iim, jjm, llm, zzm, wwm, itau, def_beha, &
                                      temp_g, MY_OPERATOR, nbindex, ind_cell_glo_glo(ijndex(:)))
...
!-
    ...
    IF (is_root_prc) THEN 
       ...
       ! Test if the variable was found in the restart file by testing the default value val_exp. 
       ! This can not be done after the scatter2D_mpi because the local 2D domain is bigger than the true values and might contain uninitialized values.
       IF(ALL(temp_g == val_exp)) THEN
          is_in_restart=.FALSE. 
       ELSE
          is_in_restart=.TRUE. 
       END IF
    ...
    CALL bcast(is_in_restart)
    IF (is_in_restart) THEN
       ! The variable was found in the restart file. Distribute it to all processes.
       CALL scatter2D_mpi(temp_g,var)
    ELSE
       ! The variable was not found in the restart file.
       ! Set the variable to val_exp so it can be tested outside the subroutine using IF (ALL(var==val_exp)).
       var=val_exp
    END IF
...
!-
    ...
    IF (grid_type==unstructured) THEN
       ...
       ENDIF
      CALL gather_unindexed(ind_cell_glo,ind_cell_glo_glo)
      IF (is_root_prc) CALL restput(fid, vname_q, iim, jjm, llm, zzm, itau, temp_g, MY_OPERATOR, &
                                     nbindex, ind_cell_glo_glo(ijndex(:)))
    ELSE  
      IF (is_root_prc) THEN
         CALL restput &
...
!-
    ...
    IF (grid_type==unstructured) THEN
       ...
       ENDIF
      CALL gather_unindexed(ind_cell_glo,ind_cell_glo_glo)
      IF (is_root_prc) CALL restput(fid, vname_q, iim, jjm, llm, zzm, wwm, itau, temp_g, MY_OPERATOR, &
                                     nbindex, ind_cell_glo_glo(ijndex(:)))
    ELSE  
      IF (is_root_prc) THEN
         CALL restput &
...
!!  =============================================================================================================================
!! SUBROUTINE: rest_o_nb_dims
!!
!>\BRIEF      Get the number of Orchidee dimensions for a given variable name in the restart file
!!
!! DESCRIPTION:	Get the number of Orchidee dimensions for a given variable name
!!              from the restart file. 
!!              - 
!!
!! \n
!_ ==============================================================================================================================
  FUNCTION rest_o_nb_dims(rest_id, var_name) RESULT (nbdims)

    INTEGER,INTENT(IN) :: rest_id
    CHARACTER(LEN=*),INTENT(IN) :: var_name

    INTEGER :: nbdims ! Output
     
    INTEGER(i_std),PARAMETER                   :: ovarnbdim_maxval=20        !! maximal # of dimensions assumed for any variable 
    INTEGER,DIMENSION(ovarnbdim_maxval)        :: vardims                !! length of each dimension of a given variable 
    INTEGER(i_std)                             :: varnbdim               !! # of dimensions of a given variable 
                                                                       !! of the stomate restart file
    LOGICAL :: is_scalar 

    CALL ioget_vdim (rest_id, var_name, ovarnbdim_maxval, varnbdim, vardims)

    ! is it scalar?
    is_scalar = ioget_var_is_scalar(vardims)
    ! DEBUG line
    !WRITE(*,*) "rest_o_nb_dims:: ", TRIM(var_name),": ", varnbdim, "-", vardims(1:varnbdim)

    IF (is_scalar) THEN
      nbdims = 1  
    ELSE
      nbdims = varnbdim - 2 ! exclude time dimension introduced by IOIPSL 
                            ! exclude merged x-y to nbp_glo
    ENDIF

  END FUNCTION rest_o_nb_dims
...
!! \n
!_ ==============================================================================================================================
  FUNCTION ioget_var_is_scalar(vardims) RESULT (is_scalar)

    INTEGER,INTENT(IN) :: vardims(:)  !! length of each dimension of a given variable 

    LOGICAL :: is_scalar ! Output
     
    ! is it scalar?
    is_scalar = ALL(vardims == 1)

  END FUNCTION  ioget_var_is_scalar


!!  =============================================================================================================================
!! SUBROUTINE: restransfer_scal
!!
!>\BRIEF      Move variable data from the input restart file to the output
!!
!! DESCRIPTION:	It applies to Real scalar values
!!
!! \n
!_ ==============================================================================================================================
  SUBROUTINE restransfer_scal(rest_id, var_name, itau)
     INTEGER,INTENT(IN)          :: rest_id, itau
     CHARACTER(LEN=*),INTENT(IN) :: var_name

     REAL(r_std),DIMENSION(1)    :: xtmp                   !! scalar read/written in restget/restput routines (unitless)

     CALL restget (rest_id, var_name, 1, 1, 1, itau, .TRUE., xtmp)
     CALL restput (rest_id, var_name, 1, 1, 1, itau, xtmp)

  END SUBROUTINE restransfer_scal

!!  =============================================================================================================================
!! SUBROUTINE: restransfer_opp_rXd
!!
!>\BRIEF       Transfer a variable from the input to the output restart file 
!!
!! DESCRIPTION: Transfer a variable from the input to the output restart file
!!              rest_id defines the output restart forcing id	
!!              Only for single processor, make sure it is wrapped in is_root_proc
!! \n
!_ ==============================================================================================================================
  SUBROUTINE restransfer_opp_r1d &
  (rest_id, vname_q, vardims, itau, nbindex, ijndex)
    IMPLICIT NONE
!-
    INTEGER, INTENT(in) :: rest_id ! output restart forcing id
    CHARACTER(LEN=*), INTENT(in) :: vname_q ! Variable name to transfer
    INTEGER, INTENT(in) :: vardims(1) ! Variable dimensions
    INTEGER, INTENT(in) :: itau ! timestep
    INTEGER, INTENT(in) :: nbindex, ijndex(nbindex) ! data info from continental gridcells to 2D
    !-----------------------------
    REAL, ALLOCATABLE, DIMENSION(:) :: var
    INTEGER :: ier

    ! vardims X(1), Y(2), remaining variables, time(last position)
    ALLOCATE( var(vardims(1)), stat=ier)
    IF (ier /= 0) CALL ipslerr(3, 'restransfer_r1d', 'var : error in memory allocation', '', '')
    !----
    CALL restget &
      &        (rest_id, vname_q, vardims(1), 1, &
      &         1, itau, .TRUE., var, "gather", nbindex, ijndex)
    CALL restput &
      &        (rest_id, vname_q, vardims(1), 1, &
      &         1, itau, var, 'scatter',  nbindex, ijndex)
    !----
    DEALLOCATE(var)
          
  END SUBROUTINE restransfer_opp_r1d


  SUBROUTINE restransfer_opp_r2d &
  (rest_id, vname_q, vardims, itau, nbindex, ijndex)
    IMPLICIT NONE
!-
    INTEGER, INTENT(in) :: rest_id
    CHARACTER(LEN=*), INTENT(in) :: vname_q
    INTEGER, INTENT(in) :: vardims(2)
    INTEGER :: itau
    INTEGER :: nbindex, ijndex(nbindex)
    !-----------------------------
    INTEGER :: ier
    REAL, ALLOCATABLE, DIMENSION(:,:) :: var
    ! vardims X(1), Y(2), remaining variables, time(last position)
    ALLOCATE( var(vardims(1), vardims(2)), stat=ier)
    IF (ier /= 0) CALL ipslerr(3, 'restransfer_r2d', 'var : error in memory allocation', '', '')
    !----
    CALL restget &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         1, itau, .TRUE., var, "gather", nbindex, ijndex)
    CALL restput &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         1, itau, var, 'scatter',  nbindex, ijndex)
  ...
  END SUBROUTINE restransfer_opp_r2d


  SUBROUTINE restransfer_opp_r3d &
  (rest_id, vname_q, vardims, itau, nbindex, ijndex)
    IMPLICIT NONE
!-
    INTEGER, INTENT(in) :: rest_id
    CHARACTER(LEN=*), INTENT(in) :: vname_q
    INTEGER, INTENT(in) :: vardims(3)
    INTEGER :: itau, ier
    INTEGER :: nbindex, ijndex(nbindex)
    !-----------------------------
    REAL, ALLOCATABLE, DIMENSION(:,:,:) :: var
    ! vardims X(1), Y(2), remaining variables, time(last position)
    ALLOCATE( var(vardims(1), vardims(2), vardims(3)), stat=ier)
    IF (ier /= 0) CALL ipslerr(3, 'restransfer_r3d', 'var : error in memory allocation', '', '')
    !----
    CALL restget &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         vardims(3), itau, .TRUE., var, "gather", nbindex, ijndex)
    CALL restput &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         vardims(3), itau, var, 'scatter',  nbindex, ijndex)
    !----
    DEALLOCATE(var)
          
  END SUBROUTINE restransfer_opp_r3d


  SUBROUTINE restransfer_opp_r4d &
  (rest_id, vname_q, vardims, itau, nbindex, ijndex)
    IMPLICIT NONE
!-
    INTEGER, INTENT(in) :: rest_id
    CHARACTER(LEN=*), INTENT(in) :: vname_q
    INTEGER, INTENT(in) :: vardims(4)
    INTEGER :: itau, ier
    INTEGER :: nbindex, ijndex(nbindex)
    !-----------------------------
    REAL, ALLOCATABLE, DIMENSION(:,:,:,:) :: var
    ! vardims X(1), Y(2), remaining variables, time(last position)
    ALLOCATE( var(vardims(1), vardims(2), vardims(3), vardims(4)), stat=ier)
    IF (ier /= 0) CALL ipslerr(3, 'restransfer_r4d', 'var : error in memory allocation', '', '')
    !----
    CALL restget &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         vardims(3), vardims(4), itau, .TRUE., var, "gather", nbindex, ijndex)
    CALL restput &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         vardims(3), vardims(4), itau, var, 'scatter',  nbindex, ijndex)
    !----
    DEALLOCATE(var)
          
  END SUBROUTINE restransfer_opp_r4d


  SUBROUTINE restransfer_opp_r5d &
  (rest_id, vname_q, vardims, itau, nbindex, ijndex)
    IMPLICIT NONE
!-
    INTEGER, INTENT(in) :: rest_id
    CHARACTER(LEN=*), INTENT(in) :: vname_q
    INTEGER, INTENT(in) :: vardims(5)
    INTEGER :: itau, ier
    INTEGER :: nbindex, ijndex(nbindex)
    !-----------------------------
    REAL, ALLOCATABLE, DIMENSION(:,:,:,:,:) :: var
    ! vardims X(1), Y(2), remaining variables, time(last position)
    ALLOCATE( var(vardims(1), vardims(2), vardims(3), vardims(4), vardims(5)), stat=ier)
    IF (ier /= 0) CALL ipslerr(3, 'restransfer_r5d', 'var : error in memory allocation', '', '')
    !----
    CALL restget &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         vardims(3), vardims(4), vardims(5), itau, .TRUE., var, "gather", nbindex, ijndex)
    CALL restput &
      &        (rest_id, vname_q, vardims(1), vardims(2), &
      &         vardims(3), vardims(4), vardims(5), itau, var, 'scatter',  nbindex, ijndex)
    !----
    DEALLOCATE(var)
          
  END SUBROUTINE restransfer_opp_r5d


!!  =============================================================================================================================
!! SUBROUTINE:  restransfer_var
!!
!>\BRIEF       Transfer a variable from the input to the output restart file
!!
!! DESCRIPTION:	Available for scalar and restXXX_opp_rXd variable types 
!! \n
!_ ==============================================================================================================================
  SUBROUTINE restransfer_var (rest_id, vname_q, itau, nbindex, ijndex)
    IMPLICIT NONE
!-
    INTEGER, INTENT(in) :: rest_id ! output restart file id
    CHARACTER(LEN=*), INTENT(in) :: vname_q ! variable name
    INTEGER :: itau, ier ! restart time step
    INTEGER :: nbindex, ijndex(nbindex) ! continental landpoints to 2D world gridpoints 
    !-----------------------------
    INTEGER(i_std)                   :: varnbdim               !! # of dimensions of a given variable 
                                                               !! of the stomate restart file
    ...
    LOGICAL :: is_var_scalar 
    INTEGER(i_std) :: orch_vardims ! orchidee variables
    CHARACTER(LEN=:), ALLOCATABLE :: msg3
    !-----------------------------

    CALL ioget_vdim (rest_id, vname_q, varnbdim_max, varnbdim, vardims)

    is_var_scalar = ioget_var_is_scalar(vardims(1:varnbdim))

    IF (is_var_scalar) THEN
      CALL restransfer_scal (rest_id, vname_q, itau)
    ELSE
      ! From restart to orchidee number of dimensions
      orch_vardims = rest_o_nb_dims(rest_id, vname_q)

      IF (orch_vardims == 1) THEN
         !----
         CALL restransfer_opp_r1d(rest_id, vname_q, (/ nbindex /), &
                                  itau, nbindex, ijndex)
          !----
      ELSE IF (orch_vardims == 2) THEN
         !----
         CALL restransfer_opp_r2d(rest_id, vname_q, (/ nbindex, vardims(3) /), &
                                itau, nbindex, ijndex)
      ELSE IF (orch_vardims == 3) THEN
         !----
         CALL restransfer_opp_r3d(rest_id, vname_q, (/ nbindex, vardims(3), vardims(4) /), &
                                itau, nbindex, ijndex)
      ELSE IF (orch_vardims == 4) THEN
         !----
         CALL restransfer_opp_r4d(rest_id, vname_q, (/ nbindex, vardims(3), vardims(4), vardims(5) /), &
                                itau, nbindex, ijndex)
      ELSE IF (orch_vardims == 5) THEN
         !----
         CALL restransfer_opp_r5d(rest_id, vname_q, (/ nbindex, vardims(3), vardims(4), vardims(5), vardims(6) /), &
                                itau, nbindex, ijndex)
      ELSE
         WRITE( msg3, '(i5)' ) orch_vardims
         CALL ipslerr(3, 'restransfer', 'Restart variable not implemented for N dimensions', &
                    & vname_q, TRIM(msg3))
      ENDIF ! orch_vardims == int(X)
   ENDIF ! is_var_scalar

          
  END SUBROUTINE restransfer_var
