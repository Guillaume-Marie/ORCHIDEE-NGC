...
!!   
!! SVN     :
!! $HeadURL: svn://forge.ipsl.fr/orchidee/trunk/ORCHIDEE/src_driver/readdim2.f90 $ 
!! $Date: 2023-07-04 15:51:03 +0200 (mar. 04 juil. 2023) $
!! $Revision: 8070 $
!! \n
!_ ================================================================================================================================
...
!- 
   ...
   ! Trying to find a way that calculates solar angle for driver files
   ! with high resolution (short timesteps).  3600.0 is what the value
   ! originally was, but I do not know where that came from.  I tried
   ! 1799.0 for FLUXNET forcing files, which results in a solar angle,
   ! but it also caused some problems with SWDown values.  Keeping this 
   ! at 3600 now, and moving it as a parameter up here to make it clear.
   REAL, PARAMETER                          :: dt_cutoff=3600.0
...
!! 0.3 Local variables
    ...
    IF ( interpol ) THEN
       ...
       IF (printlev_loc >= 1) WRITE(numout,*) 'Standard PRINTLEV= ', printlev
  ...
  IMPLICIT NONE
   ...
   ELSEIF ( weathergen ) THEN
      ...
      IF ( (itauin == 0).AND.(itau_split == 0) ) THEN
         ! Initialization of weathgen_main
      ...
      IF ( (itauin == 0).AND.(itau_split == 0) ) THEN
         ...
         ! Initialization of height levels done at first call
         zlev_fixed = 2.0
         CALL getin_p('HEIGHT_LEV1', zlev_fixed)
         
         zlevuv_fixed = 10.0
         CALL getin_p('HEIGHT_LEVW', zlevuv_fixed)

      ENDIF

      !- Always set fixed height levels for case weathergen
      zlev(:,:)=zlev_fixed             !level for t and q
      zlevuv(:,:)=zlevuv_fixed         !level for u and v
...
!jur, 
   ...
   REAL(r_std), DIMENSION(1,1) :: lon_temp  ! Only used in a special case
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!--- With the use of ORCHIDEE-CN-CAN, the solar angle needs to be provided at every timestep.
!--- Calculation of the absorption of light in the canopy (and therefore GPP) depends on this angle.
!--- Previously, the solar angle was not always computer (for example, for driver files
!--- with timesteps of less than half an hour.  The other problem, though, is that driver file
!--- time (often local time) does not always match ORCHIDEE time (UTC) for a single site.
!--- We treat this specific case here.  We may still have problems with daily_interpol, I believe.
!--- If dt_force is less than dt_cutoff (previously set to 3600), the solar angle will not
!--- be calculated below.
   IF ( dt_force .LE. dt_cutoff )  THEN

      ! First, the case we have tested.  The forcing file timestep equals the model timestep.
      IF(split .EQ. 1)THEN
         IF(iim .EQ. 1 .AND. jjm .EQ. 1)THEN
            ...
            julian = julian_for+dt_force/one_day
            ! By temporarily setting the longitude to zero degrees, we force UTC.
            lon_temp(:,:)=0.0_r_std
            CALL solarang (julian, julian0, iim, jjm, lon_temp, lat, coszang)
         ELSE
            CALL ipslerr_p(3,'forcing_read_interpol', &
                 'The forcing file is high frequency but not a single point!', &
                 'Not sure how to deal with this in regards to the solar angle.',&
                 'Continuing may result in points with low or zero GPP.') 
         ENDIF
      ELSE
         CALL ipslerr_p(3,'forcing_read_interpol', &
              'The forcing file is high frequency but not the same as the model timestep!', &
              'Not sure how to deal with this in regards to the solar angle.',&
              'Continuing may result in points with low or zero GPP.') 
      ENDIF
   ENDIF
